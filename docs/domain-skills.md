...
This is torch's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
torch copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every torch project I work on"), explicitly run
...
- **Per-project**: `~/.torch/projects/<slug>/learnings.jsonl` — same JSONL
  file the `/learn` skill uses. Domain skills are `type:"domain"` rows.
- **Global**: `~/.torch/global-domain-skills.jsonl` — only `state:"global"`
  rows.
