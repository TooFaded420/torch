...
Every error message `torch-brain-*` can print, with problem, cause, and fix.
...
**Problem.** You're on a machine that has `~/.torch-brain-remote.txt` (copied
from another machine) but no local git repo at `~/.torch/.git`.

**Cause.** You've set up GBrain sync elsewhere and your torch hasn't been
...
torch-brain-restore
```
This pulls the repo into `~/.torch/` and re-registers merge drivers.

If you don't want to restore here, dismiss the hint with:
```bash
torch-config set artifacts_sync_mode_prompted true
```
...
**Fix.** Look at `~/.torch/.brain-sync-status.json` for more detail, or run:
```bash
cd ~/.torch && git status && git push origin HEAD
```
...
## `torch-brain-init: ~/.torch/.git is already a git repo pointing at <url>`

**Problem.** You tried to init with a remote URL that doesn't match the
existing one.

**Cause.** You already ran `torch-brain-init` with a different remote.

**Fix.** Either:

- Use the existing remote: run `torch-brain-init` without `--remote`, or
  with the matching URL.
- Switch remotes: `torch-brain-uninstall` first, then re-init with the new
...
## `torch-brain-init: failed to create or find '<name>'`
...
name:
```bash
torch-brain-init --remote git@github.com:YOURUSER/custom-name.git
```

---

## `torch-brain-restore: ~/.torch/.git already points at <url>`
...
**Fix.** `torch-brain-uninstall`, then re-run `torch-brain-restore <url>`.

---

## `torch-brain-restore: ~/.torch/ has existing allowlisted files that would be clobbered`

**Problem.** You're trying to restore, but `~/.torch/` already contains
learnings or plans that would be overwritten.

**Cause.** Either (a) this machine has accumulated state from a pre-sync
torch session, or (b) a previous failed restore left partial state.

**Fix (three options).**

1. **If this machine's state should become the new truth**: run
   `torch-brain-init` instead of restore — this creates a brand-new brain
   repo from this machine's state.

2. **If you want to adopt the remote and discard this machine's state**:
   back up `~/.torch/projects/` first, then remove the offending files and
   re-run restore.

3. **If you want to merge**: there's no automatic merge for this. Manually
   copy learnings from `~/.torch/` into your running torch on a machine
   with sync already on, then restore here.

---

## `torch-brain-restore: <url> does not look like a torch-brain repo`
...
**Fix.** Verify the URL. If it's correct, run `torch-brain-init --remote
...
1. `torch-brain-sync --status` — is mode `off`?
2. `~/.torch/.git` exists?
3. `torch-config get artifacts_sync_mode` — should be `full` or `artifacts-only`.
4. The file you expect to sync — is it in the allowlist?
   `cat ~/.torch/.brain-allowlist`
...
```bash
torch-brain-sync --discover-new
torch-brain-sync --once
```
to force a drain.
