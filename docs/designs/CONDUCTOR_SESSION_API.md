...
When Claude controls your real browser via CDP (torch `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

torch's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the torch Chrome extension Side Panel that shows:
...
## What torch builds (extension side)
