...
**Date:** 2026-03-28
**CEO Plan:** ~/.torch/projects/garrytan-torch/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The torch Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any torch component can call these
...
```
AskUserQuestion:
  "torch just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all torch users. Can we
...
2. Cache to ~/.torch/models/browsesafe-bench/ (not re-downloaded in CI)
...
- torch becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But torch already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.torch/models/
