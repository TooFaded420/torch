# torch Browser V0 — The AI-Native Development Browser
...
consumer browser and bolts AI onto it. torch Browser inverts this. It starts
...
for text editors, torch Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the torch
...
and run any torch skill. All without touching a terminal.

```
torch Browser.app (389MB, 189MB DMG)
...
When the extension side panel hits hard API limits, torch Browser ships to
...
Every torch skill becomes a browser capability.
...
| **torch Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

torch Browser doesn't compete with consumer browsers. It competes with the
...
| `torch_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
The 12-month ideal: you open torch Browser, it detects your project, starts
