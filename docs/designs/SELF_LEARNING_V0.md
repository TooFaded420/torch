# Design: torch Self-Learning Infrastructure
...
torch runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every torch skill can read and
write. The goal: after 20 sessions on the same codebase, torch knows every
...
YC founders building with AI. The people who run torch on real codebases 20+ times
...
| **torch** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

torch has four distinct persistence layers. They share storage patterns
(JSONL in `~/.torch/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "torch Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.torch/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via torch-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read torch-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via torch-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.torch/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), torch suggests what you probably want next.
...
- Timeline binaries: `bin/torch-timeline-log` and `bin/torch-timeline-read`.
...
**Headline:** torch respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of torch's approach. We adapted every concept to fit torch's template system, voice, and architecture rather than porting directly.
