...
T+1-3s    Extension loads in Chromium
            ...
            │     └── Does NOT fire torch-extension-ready (waits for sidebar)
...
4. server.ts /sidebar-command handler
   ...
   └── Appends JSON to ~/.torch/sidebar-agent-queue.jsonl
...
3. content.js onMessage handler
   └── document.dispatchEvent(new CustomEvent('torch-extension-ready'))

4. welcome.html script
   └── addEventListener('torch-extension-ready', () => arrow.classList.add('hidden'))
```
...
Server starts → AUTH_TOKEN = crypto.randomUUID()
    ...
    │
    └── sidebar-agent.ts refreshToken() → reads from .torch/browse.json
...
| Agent stale token | Agent runs but no events appear in sidebar | sidebar-agent has old token from .torch/browse.json | Agent re-reads token before each event POST |
...
| Queue file | `~/.torch/sidebar-agent-queue.jsonl` | Filesystem |
| State file | `.torch/browse.json` | Filesystem |
| Chat log | `~/.torch/sessions/<id>/chat.jsonl` | Filesystem |
