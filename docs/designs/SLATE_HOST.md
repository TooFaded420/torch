...
torch projects... CLAUDE.md works as-is.
...
### Critical env vars for torch integration
...
Without native `.slate/` publishing, torch skills vanish when this flag is set.
...
torch skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install torch for Claude Code
...
- Host names duplicated in `bin/torch-platform-detect`, `bin/torch-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/torch-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/torch-platform-detect` doesn't detect Slate
