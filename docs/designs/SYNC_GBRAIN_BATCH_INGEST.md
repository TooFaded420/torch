...
**Estimated effort:** human ~3 days / CC+torch ~2 hr
...
exfiltration boundary is `torch-brain-sync` (bin/torch-brain-sync:78-110,
...
for 5031). That's a gbrain-side perf issue, not torch architecture.
...
1. **gbrain import perf on large dirs** — investigate why 5031 files
   ...
   FTS index updates without batching. Lives in gbrain, not torch.
...
Two compounding bugs in `bin/torch-memory-ingest.ts`:

1. **Subprocess-per-file architecture.** The ingest loop at line 911 walks
   1,841 files in `~/.torch/projects/` and spawns two subprocesses per file:
   - `gitleaks detect --no-git --source <path>` — 46ms cold start (`lib/torch-memory-helpers.ts:157`)
   - `gbrain put <slug>` — 329ms cold start (`bin/torch-memory-ingest.ts:823`)
   - Per-file floor: 375ms × 1841 = 690s (11.5 min) of pure subprocess startup
     before any actual work happens.

2. **Kill-no-save timeout.** Orchestrator at `bin/torch-gbrain-sync.ts:442`
   enforces a 35-min timeout. When it fires, `spawnSync` returns
   `result.status === null`, the child gets SIGTERM, and the in-memory
   ingest state never flushes to `~/.torch/.transcript-ingest-state.json`.
...
| Metric | Value | Source |
|---|---|---|
| Files in walkAllSources | 1,841 | `find ~/.torch/projects -type f \( -name "*.md" -o -name "*.jsonl" \)` |
...
Take everything in `ingestPass` (lines 899-988 of `bin/torch-memory-ingest.ts`)
...
- Created at `~/.torch/.staging-ingest-${pid}-${ts}/`
...
redaction policy in `lib/torch-memory-helpers.ts`).
...
### Step 6: SIGTERM handler in `torch-memory-ingest.ts`
...
In `bin/torch-gbrain-sync.ts:444`:
...
`~/.torch/.ingest-skip-list.json` so the next prepare stage skips that file
entirely. Avoids re-staging a file that will always fail. User can review
the skip list with a new `torch-memory-ingest --skip-list` flag.
...
3. **Benchmark gate (periodic, paid):**
   ...
   - Test fixture: copy of `~/.torch/projects/` snapshot for repeatable timing.

## Rollback strategy

- New `--legacy-ingest` flag on `torch-memory-ingest` keeps the old
  per-file path callable for one release cycle.
- If batch path regresses on a real corpus, set
  `torch-config set memory_ingest_path legacy` to revert without redeploy.
...
4. **Cross-worktree concurrency.** `~/.torch/.staging-ingest-${pid}-${ts}/`
   is pid-namespaced so two concurrent /sync-gbrain runs don't collide.
   But the orchestrator already holds a lock at `~/.torch/.sync-gbrain.lock`
...
6. **Should we deprecate `gbrain put` for memory entirely?** The legacy
   ...
   ingestion? Probably yes (for `~/.torch/.transcript-ingest-state.json`
   updates triggered outside the orchestrator), but worth confirming.

## What this isn't

- Not a gbrain CLI change. All work is in torch.
