...
torch already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has torch's artifact architecture.

## The Insight

torch already writes structured artifacts to `~/.torch/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.torch/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.torch/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle torch artifacts)
