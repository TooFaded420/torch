...
- torch installed (`./setup` complete; verify with `which torch` or by typing `/` in Claude Code and seeing skills listed)
...
- **Why torch uses the Diataxis framework:** [explanation-diataxis-in-torch.md](./explanation-diataxis-in-torch.md)
