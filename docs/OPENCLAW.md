# torch x OpenClaw Integration

torch integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. torch provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates torch-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates torch-full
       │           └── torch installed at      │   (complete pipeline)
       │               ~/.claude/skills/torch  │
...
OpenClaw decides at spawn time which tier of torch support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No torch context injected |
| **Medium** | Multi-file features, refactors | torch-lite CLAUDE.md appended |
| **Heavy** | Specific torch skill needed | "Load torch. Run /X" |
| **Full** | Complete features, objectives, projects | torch-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | torch-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-torch-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY torch skill,
...
torch-lite/full as a new section. Do not replace the repo's existing instructions.

## What torch generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### torch-lite (Medium tier)
`openclaw/torch-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### torch-full (Full tier)
`openclaw/torch-full-CLAUDE.md` — chains existing torch skills:
...
### torch-plan (Plan tier)
`openclaw/torch-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `torch-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `torch-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `torch-openclaw-investigate` — Operational debugging (4-phase methodology)
- `torch-openclaw-retro` — Operational retrospective (weekly review)
...
For OpenClaw users: tell your OpenClaw agent "install torch for openclaw."

The agent should:
1. Install torch-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For torch developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from torch (OpenClaw has its own)
