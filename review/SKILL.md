...
preamble-tier: 4
version: 1.0.0
description: Pre-landing PR review. (torch)
allowed-tools:
  - Bash
  - Read
  - Edit
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/torch/bin/torch-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/torch/bin/torch-question-log '{"skill":"review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
QUEUE_JSON=$(bun run bin/torch-next-version \
  --base "$BASE_BRANCH" \
  --bump patch \
...
```bash
source <(~/.claude/skills/torch/bin/torch-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/torch/bin/torch-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/torch/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/torch/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/torch/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/torch/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/torch/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/torch/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/torch/review/design-checklist.md`
...
```bash
~/.claude/skills/torch/bin/torch-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/torch/review/specialists/red-team.md`
...
```bash
~/.claude/skills/torch/bin/torch-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/torch/bin/torch-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
