/**
 * torch-artifacts-init — provider-selection + brain-admin-hookup tests.
 *
 * Mirrors the torch-brain-init-gh-mock.test.ts pattern: install fake gh /
 ...
 * ~/.torch-artifacts-remote.txt, (c) prints the "Send this to your brain
...
const INIT_BIN = path.join(ROOT, 'bin', 'torch-artifacts-init');
...
function makeFakeGh(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'already-exists' | 'fail'; webUrl?: string } = {}) {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/torch-artifacts-testuser`;
...
function makeFakeGlab(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'fail'; webUrl?: string } = {}) {
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/torch-artifacts-testuser';
  const script = `#!/bin/bash
...
describe('torch-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with torch-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('torch-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    // No gh, no glab fakes.
    const r = run([], { input: 'https://github.com/testuser/torch-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('torch-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.torch-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/torch-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.torch-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/torch-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/torch-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.torch-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/torch-artifacts-testuser');
  });
  ...
  test('configures git origin with SSH form (derived from canonical HTTPS)', () => {
    ...
    expect(remote.stdout.trim()).toBe('git@github.com:testuser/torch-artifacts-testuser.git');
  });
});

describe('torch-artifacts-init brain-admin hookup printout (codex Finding #3)', () => {
  ...
  test('--url-form-supported true prints the one-liner with --url', () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Send this to your brain admin');
    expect(r.stdout).toContain('gbrain sources add torch-artifacts-testuser --url');
  ...
  test('the gbrain command line uses canonical HTTPS, not SSH', () => {
    ...
    expect(gbrainLine).toContain('https://github.com/testuser/torch-artifacts-testuser');
...
describe('torch-artifacts-init idempotency', () => {
  test('--remote <url> bypasses provider selection entirely', () => {
    makeFakeGh({});
    const r = run(['--remote', 'https://github.com/testuser/torch-artifacts-testuser']);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/torch-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/torch-artifacts-testuser']);
