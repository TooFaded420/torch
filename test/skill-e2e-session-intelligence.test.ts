...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    torchHome = path.join(workDir, '.torch-home');
    ...
    // Compute slug (same logic as torch-slug without git remote)
  ...
  // Write a timeline event via torch-timeline-log, read it back via torch-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(torchHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'torch-timeline-log');
    const readBin = path.join(workDir, 'bin', 'torch-timeline-read');
    const env = { ...process.env, TORCH_HOME: torchHome };
    ...
    // Verify torch-timeline-read output includes the events
...
IMPORTANT:
- Use TORCH_HOME="${torchHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/torch/bin/.
  Replace any references to ~/.claude/skills/torch/bin/ with ./bin/ when running commands.
