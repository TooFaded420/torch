...
function setupWorkdir(suffix: string): { workDir: string; torchHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const torchHome = path.join(workDir, '.torch-home');
  ...
  for (const script of [
    'torch-timeline-log', 'torch-timeline-read', 'torch-slug',
    'torch-learnings-log', 'torch-learnings-search',
    'torch-update-check', 'torch-config', 'torch-repo-mode',
  ]) {
...
Environment:
- Use torch_HOME="${torchHome}" for all torch bin scripts.
...
`);
  ...
  return { workDir, torchHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(torchHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(torchHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The torch_HOME / ./bin bash setup that used to be in
    ...
    const checkpointDir = path.join(torchHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(torchHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(torchHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(torchHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('delegates');
    seedSave(torchHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(torchHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(torchHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(torchHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(torchHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, torchHome, slug } = setupWorkdir('list-all');
