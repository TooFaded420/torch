 ...
 * `.gbrain-source` to torch's own `.gitignore`. Without the consumer-side
...
import { ensureGbrainSourceGitignored } from "../bin/torch-gbrain-sync";

describe("ensureGbrainSourceGitignored", () => {
  let root: string;

  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "torch-gbrain-gitignore-"));
  });
