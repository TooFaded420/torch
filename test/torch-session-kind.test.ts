/**
 * torch-session-kind — classifies the session so skills know whether a human can
...
const BIN = path.resolve(__dirname, '..', 'bin', 'torch-session-kind');
...
describe('torch-session-kind', () => {
  test('OPENCLAW_SESSION → spawned (highest precedence)', () => {
    ...
    expect(kind({ OPENCLAW_SESSION: '1', torch_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('spawned');
  });

  test('torch_HEADLESS → headless', () => {
    expect(kind({ torch_HEADLESS: '1' })).toBe('headless');
  });
  ...
  test('torch_HEADLESS beats CONDUCTOR (explicit override wins)', () => {
    expect(kind({ torch_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('headless');
  });
  ...
  test('empty torch_HEADLESS is treated as unset (interactive)', () => {
    ...
    expect(kind({ torch_HEADLESS: '' })).toBe('interactive');
  });
});
