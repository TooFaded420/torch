/**
 * torch-artifacts-url — URL canonicalization helper.
 *
...
const URL_BIN = path.join(ROOT, 'bin', 'torch-artifacts-url');
...
describe('torch-artifacts-url', () => {
  ...
  test('--to https from ssh:// scheme (gitlab self-hosted style)', () => {
    const r = run(['--to', 'https', 'ssh://git@gitlab.example.org/team/torch-artifacts-team.git']);
    expect(r.stdout).toBe('https://gitlab.example.org/team/torch-artifacts-team');
  });
  ...
  test('--owner-repo extracts the path segment', () => {
    ...
    expect(run(['--owner-repo', 'git@github.com:team/torch-artifacts-team.git']).stdout)
      .toBe('team/torch-artifacts-team');
  });
