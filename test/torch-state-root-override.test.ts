/**
 * torch_STATE_ROOT override — verifies the 3 plan-tune bins honor
 * torch_STATE_ROOT as a higher-priority override over torch_HOME.
 *
 * Surfaced by plan-tune cathedral D16 (Codex outside voice): tests can't
 * isolate from real ~/.torch today because the bins ignore STATE_ROOT.
 * Without this override, the cathedral's E2E + integration tests would
 * silently pollute the user's real profile.
 *
 * Contract:
 *   - torch_STATE_ROOT set → bins write under STATE_ROOT (HOME ignored).
 *   - Only torch_HOME set → bins write under HOME (existing behavior).
 *   - Neither set → falls back to $HOME/.torch (existing behavior).
...
const BIN_LOG = path.join(ROOT, 'bin', 'torch-question-log');
const BIN_PREF = path.join(ROOT, 'bin', 'torch-question-preference');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-state-'));
  homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-home-'));
});
...
function runBin(
  ...
  if (env.torch_STATE_ROOT === undefined) delete cleaned.torch_STATE_ROOT;
  if (env.torch_HOME === undefined) delete cleaned.torch_HOME;
...
describe('torch-question-preference honors torch_STATE_ROOT', () => {
  test('STATE_ROOT set → preferences file lives under STATE_ROOT', () => {
    const write = runBin(
      ...
      { torch_STATE_ROOT: stateRoot, torch_HOME: undefined },
    );
...
describe('torch-developer-profile honors torch_STATE_ROOT', () => {
