// Pure-function tests for bin/torch-next-version.
...
describe("resolveVersionPath (monorepo VERSION-path support)", () => {
  test("CLI flag wins over everything", () => {
    ...
    try {
      mkdirSync(join(dir, ".torch"));
      writeFileSync(join(dir, ".torch", "version-path"), "config/VERSION\n");
  ...
  test(".torch/version-path config is picked up", () => {
    const dir = mkdtempSync(join(tmpdir(), "nextver-"));
    try {
      mkdirSync(join(dir, ".torch"));
      writeFileSync(join(dir, ".torch", "version-path"), "Tinas Second Brain/health-tracker/VERSION\n");
  ...
  test("trims whitespace and ignores blank lines after the first", () => {
    ...
    try {
      mkdirSync(join(dir, ".torch"));
      writeFileSync(join(dir, ".torch", "version-path"), "  apps/web/VERSION  \n\n# comment-ish line\n");
  ...
  test("empty config file falls back to default VERSION", () => {
    ...
    try {
      mkdirSync(join(dir, ".torch"));
      writeFileSync(join(dir, ".torch", "version-path"), "\n");
  ...
  test("empty override string falls back to config/default", () => {
    ...
    try {
      mkdirSync(join(dir, ".torch"));
      writeFileSync(join(dir, ".torch", "version-path"), "subproj/VERSION\n");
