 ...
 * on PATH), runs a cheap torch skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('torch skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh torch_HOME with artifacts_sync_mode_prompted=false.
    const torchHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-torch-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-bin-'));

    // Seed the config so the gate's condition passes.
    fs.writeFileSync(
      path.join(torchHome, 'config.yaml'),
    ...
    // don't inherit them.
    const origtorchHome = process.env.torch_HOME;
    ...
    try {
      ...
      if (origtorchHome === undefined) delete process.env.torch_HOME;
      else process.env.torch_HOME = origtorchHome;
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const torchHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
