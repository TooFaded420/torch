/**
 * torch-slug cache-read sanitization.
 *
 * `eval "$(torch-slug)"` is how callers load SLUG/BRANCH. The compute and
...
describe('torch-slug cache-read sanitization', () => {
  test('a poisoned cache file cannot inject shell metacharacters into output', () => {
    ...
    try {
      const cacheDir = path.join(home, '.torch', 'slug-cache');
