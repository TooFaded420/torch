/**
 * Unit tests for torch-upgrade/migrations/v1.37.0.0.sh — split-engine notice.
 *
...
function makeEnv(opts: {
  ...
  const claudeSkillsBin = join(home, ".claude", "skills", "torch", "bin");
  const claudeJson = join(home, ".claude.json");
  const gbrainConfig = join(gbrainDir, "config.json");
  const configBin = join(claudeSkillsBin, "torch-config");
  ...
  // Fake torch-config: returns "true" iff opted-out (matches the real bin's
...
`,
  ...
  return {
    ...
    home,
    torchHome,
    doneTouch: join(torchHome, ".migrations", "v1.37.0.0.done"),
    claudeJson,
    gbrainConfig,
    configBin,
...
function runMigration(env: MigEnv): { stdout: string; stderr: string; exitCode: number } {
  const result = spawnSync("bash", [MIGRATION], {
    ...
    env: {
      ...
      // The script looks for torch-config at $HOME/.claude/skills/torch/bin
...
describe("torch-upgrade/migrations/v1.37.0.0.sh", () => {
