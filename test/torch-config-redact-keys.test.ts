/**
 * Config keys for redaction (T12). Verifies torch-config knows the two new
...
const CONFIG = path.resolve(import.meta.dir, "..", "bin", "torch-config");
let home: string;

function cfg(args: string[]): { code: number; out: string; err: string } {
  const r = spawnSync(CONFIG, args, {
    encoding: "utf8",
    env: { ...process.env, torch_HOME: home },
  });
