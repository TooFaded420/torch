/**
 * Regression: no stale `torch-brain-init`, `gbrain_sync_mode`, or
 * `~/.torch-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`torch-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'torch-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'torch-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/torch-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/torch-config comment explicitly preserves the rename note.
  'bin/torch-config',
  ...
  'bin/torch-brain-restore',
  'bin/torch-gbrain-source-wireup',
  'bin/torch-brain-uninstall',
  ...
  // torch-upgrade.test.ts may exercise old migration behavior.
  'test/torch-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-torch-brain-refs.test.ts',
  ...
  'test/torch-artifacts-init.test.ts',
...
describe('no stale torch-brain refs (v1.27.0.0 rename)', () => {
