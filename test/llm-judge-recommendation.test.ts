...
A) Server-side (gbrain ships the smarts)
  ...
  ❌ Cross-repo work; gbrain release tied to torch release; slower V1
B) Client-side (torch ships the smarts) (recommended)
  ✅ Ships entirely in torch — no gbrain release dependency; faster V1
...
describeIfSelected('judgeRecommendation rubric sanity', ['llm-judge-recommendation'], () => {
  testIfSelected('llm-judge-recommendation', async () => {
    ...
    const good5 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose C because hybrid ships V1 in torch-only without blocking on cross-repo gbrain coordination, and locks the migration path before other agents take a hard dependency.',
    ));
    ...
    const good4 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose B because client-side composition uses MCP tools that already exist in torch and avoids any gbrain release dependency for V1.',
    ));
