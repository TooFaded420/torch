...
(gbrain ships the smarts), client-side (torch ships the smarts), and a hybrid
...
Proceed directly to Phase 4 (Alternatives Generation). Generate 2-3 distinct architectural approaches that differ in KIND (not in coverage). Realistic shapes for this project:
  ...
  B) Client-side: torch ships a helper (bin/torch-brain-context-load) that composes salience client-side from existing MCP tools.
  C) Hybrid: V1 client-side in torch; V1.5 promotes to gbrain server-side once the salience signal is validated.
