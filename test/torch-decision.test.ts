/**
 * Unit tests for lib/torch-decision.ts — event-sourced decision memory model.
 */
...
describe("decisionPaths", () => {
  it("derives log/snapshot/archive under the project slug", () => {
    const p = decisionPaths("garrytan-torch", "/tmp/gs");
    expect(p.log).toBe("/tmp/gs/projects/garrytan-torch/decisions.jsonl");
    expect(p.snapshot).toBe("/tmp/gs/projects/garrytan-torch/decisions.active.json");
    expect(p.archive).toBe("/tmp/gs/projects/garrytan-torch/decisions.archive.jsonl");
  });
});
