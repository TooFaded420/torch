/**
 * torch-gbrain-repo-policy — per-remote trust-tier policy store.
 *
 ...
 *
 * Each test uses a temp torch_HOME so nothing leaks into the user's real ~/.torch.
 */
...
const BIN = path.join(ROOT, 'bin', 'torch-gbrain-repo-policy');
