/**
 * Unit tests for lib/torch-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "torch-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.torch/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.torch_HOME;
    testHome = mkdtempSync(join(tmpdir(), "torch-test-home-"));
    process.env.torch_HOME = testHome;
  });

  afterAll(() => {
    if (savedHome === undefined) delete process.env.torch_HOME;
    else process.env.torch_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    ...
    testHome = mkdtempSync(join(tmpdir(), "torch-test-engine-"));
    testGbrainHome = mkdtempSync(join(tmpdir(), "torch-test-gbrain-"));
    process.env.torch_HOME = testHome;
  ...
  it("writes a cache file at ~/.torch/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("torch-memory-helpers.detectEngineTier");
  });
  ...
  it("parses schema_version:2 doctor JSON via the exec path (regression for #1418)", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "torch-gbrain-shim-"));
