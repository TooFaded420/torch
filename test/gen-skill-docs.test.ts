...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root torch' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).not.toContain('torch_contributor');
    expect(content).not.toContain('contributor-logs');
    expect(content).toContain('Operational Self-Improvement');
    expect(content).toContain('torch-learnings-log');
    expect(content).toContain('torch-learnings-search --limit 3');
  });
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.torch/analytics');
  });
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // resolver gained the <torch-qid:...> marker convention + the
  ...
  test('slim voice section preserves the torch voice contract', () => {
    const content = readSkillUnion('plan-eng-review'); // carved: review body moved to section
    const voice = extractMarkdownSection(content, '## Voice');
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      expect(content).toContain("find ~/.torch/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('torch-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('torch REVIEW REPORT');
    });
  }
...
describe('make-pdf setup ordering', () => {
  test('MAKE-PDF SETUP appears before generic preamble footer sections', () => {
    ...
    const telemetryIdx = content.indexOf('## Local analytics (run last)');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = readSkillUnion('office-hours'); // carved: Phase 5/6 prose moved to section
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'torch-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('torch-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('torch-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/torch' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/torch');
...
describe('INVOKE_SKILL resolver', () => {
  ...
  test('INVOKE_SKILL output includes default skip list', () => {
    ...
    expect(ceoContent).toContain('Local analytics (run last)');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('torch')) {
        skills.push({ dir: '.', codexName: 'torch' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('torch-') ? entry.name : `torch-${entry.name}`;
  ...
  test('root torch bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "torch"');
    expect(content).toContain('Use $torch to locate the bundled torch skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is torch, others are torch-{dir}', () => {
    // Root → torch
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch', 'SKILL.md'))).toBe(true);
    // Subdirectories → torch-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-ship', 'SKILL.md'))).toBe(true);
    // torch-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: torch-torch-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-torch-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'torch-codex'))).toBe(false);
  });

  test('Codex output includes Claude outside-voice skill with read-only boundary', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'torch-claude', 'SKILL.md'), 'utf-8');
    expect(content).toContain('claude -p');
    expect(content).toContain('mktemp /tmp/torch-claude-prompt-');
    expect(content).toContain('mktemp /tmp/torch-claude-diff-');
    expect(content).not.toContain('/tmp/torch-claude-diff-$$');
  ...
  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'torch-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'torch-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'torch-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['torch-careful', 'torch-freeze', 'torch-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global torch roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'torch-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('torch_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/torch');
    expect(content).toContain('$torch_BIN/torch-config');
    expect(content).toContain('$torch_ROOT/torch-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/torch/bin/torch-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/torch/review/ (not torch-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/torch-review
    // but setup puts sidecars under .agents/skills/torch/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'torch-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use torch/review/ path
    expect(content).toContain('.agents/skills/torch/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference torch-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/torch-review/checklist.md');
  });

  test('sidecar paths in ship skill point to torch/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'torch-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/torch/review/');
      expect(content).not.toContain('.agents/skills/torch-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/torch/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/torch-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/torch → $torch_ROOT
    expect(content).not.toContain('~/.claude/skills/torch');
    expect(content).toContain('$torch_ROOT');

    // Rule 2: .claude/skills/torch → .agents/skills/torch
    expect(content).not.toContain('.claude/skills/torch');

    // Rule 3: .claude/skills/review → .agents/skills/torch/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('torch-config') || content.includes('torch-update-check') || content.includes('torch-telemetry-log')) {
        expect(content).toContain('$torch_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('torch-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/torch');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the torch Codex host install path.
      if (skill.dir !== 'pair-agent' && skill.dir !== 'codex' && skill.dir !== 'autoplan') {
        expect(content).not.toContain('~/.codex/');
      }
      // torch-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'torch-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'torch-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'torch-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('torch')) skills.push({ dir: '.', factoryName: 'torch' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('torch-') ? entry.name : `torch-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['torch-ship', 'torch-land-and-deploy', 'torch-guard', 'torch-careful', 'torch-freeze', 'torch-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['torch-qa', 'torch-review', 'torch-investigate', 'torch-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'torch-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'torch-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'torch-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'torch-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('torch_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/torch');
    expect(content).toContain('$torch_BIN/torch-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage outside repo-root sidecar symlinks', () => {
        ...
        for (const skill of skills) {
          // Dev installs may mount the repo root at host/skills/torch as a runtime
      ...
      test('generates Claude outside-voice skill for external hosts', () => {
        const skillMd = path.join(hostDir, 'torch-claude', 'SKILL.md');
        expect(fs.existsSync(skillMd)).toBe(true);
        const content = fs.readFileSync(skillMd, 'utf-8');
        expect(content).toContain('claude -p');
        expect(content).toContain('--disable-slash-commands');
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'torch-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('_link_or_copy "$torch_DIR" "$CODEX_torch"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_torch_DIR=');
    expect(setupContent).toContain('SOURCE_torch_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_torch="$INSTALL_torch_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_torch_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_torch_DIR"');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under torch/)
    ...
    expect(fnBody).toContain('_link_or_copy "$torch_dir/$dir_name/SKILL.md" "$target/SKILL.md"');
  });
  ...
  test('setup links root torch skill through a thin Claude wrapper alias', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('_torch-command');
    expect(fnBody).toContain('_link_or_copy "$torch_dir/SKILL.md" "$target/SKILL.md"');

    const claudeSection = setupContent.slice(
      setupContent.indexOf('# 4. Install for Claude'),
      setupContent.indexOf('# 5. Install for Codex')
    );
    expect(claudeSection).toContain('link_claude_root_skill_alias "$SOURCE_torch_DIR" "$INSTALL_SKILLS_DIR"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/torch from being linked as a skill
  test('link_codex_skill_dirs skips the torch sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "torch" ] && continue');
  });

  // T2: Dynamic $torch_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic torch_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'torch-ship');
    ...
    expect(content).toContain('torch_ROOT=');
    expect(content).toContain('$torch_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/torch');
  });
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_torch="$OPENCODE_SKILLS/torch"');
  });

  test('setup installs OpenCode skills into a nested torch runtime root', () => {
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('torch/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('torch-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('_link_or_copy "$torch_dir" "$codex_torch"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/torch', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.torch/repos/torch');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies torch- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="torch-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // torch-* dirs should keep their name (e.g., torch-upgrade stays torch-upgrade)
    expect(fnBody).toContain('torch-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only torch-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('torch-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('torch_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('torch-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // torch-upgrade is always called torch-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /torch-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains analytics start block', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('_TEL_START');
    expect(content).toContain('_SESSION_ID');
    expect(content).toContain('ANALYTICS:');
    expect(content).toContain('torch-config get telemetry');
  });

  test('generated SKILL.md points at GitHub issues instead of asking for telemetry', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('https://github.com/h3cz/torch/issues');
    // The old opt-in consent flow is gone along with the remote endpoint.
    expect(content).not.toContain('.telemetry-prompted');
    expect(content).not.toContain('Help torch get better');
    expect(content).not.toContain('torch-config set telemetry community');
  });

  test('generated SKILL.md contains the local-analytics epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Local analytics (run last)');
    expect(content).toContain('torch-telemetry-log');
  ...
  test('analytics blocks appear in all skill files that use PREAMBLE', () => {
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "torch"
  test('every SKILL.md.tmpl description contains "torch"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('torch');
    }
  });
  ...
  // #467 — preamble JSONL writes are gated by the analytics setting
  test('preamble JSONL writes are inside the analytics conditional', () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('skill-usage.jsonl') && lines[i].includes('>>')) {
        // Look backwards for an analytics conditional within 5 lines
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/torch symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('torch-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-prefix-warn-'));
    try {
      // Create a fake ~/.torch/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const faketorch = path.join(fakeHome, '.torch');
      fs.mkdirSync(faketorch, { recursive: true });
      fs.writeFileSync(path.join(faketorch, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('torch-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const faketorch = path.join(fakeHome, '.torch');
      fs.mkdirSync(faketorch, { recursive: true });
      fs.writeFileSync(path.join(faketorch, 'config.yaml'), 'skill_prefix: false\n');
...
// torch REVIEW REPORT report-at-bottom contract — verifies the prompt-text
...
describe('torch REVIEW REPORT delete-then-append flow', () => {
...
describe('LEARNINGS_SEARCH resolver: query parameter', () => {
  ...
  test('claude host + query=foo bar → both cross-project and project-scoped branches contain --query', () => {
    ...
    const lines = out.split('\n').filter(l => l.includes('torch-learnings-search'));
  ...
  test('codex host + query=foo bar → codex bash variant contains --query', () => {
    ...
    expect(out).toContain('$torch_BIN/torch-learnings-search');
  });
...
describe('EXIT PLAN MODE GATE placement', () => {
  ...
  // already contains `## torch REVIEW REPORT` inside a markdown example fence,
  // and the gate text itself shows `## torch REVIEW REPORT` inside a fence too.
...
describe('torch REVIEW REPORT mandatory unresolved-decisions status', () => {
