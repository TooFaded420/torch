...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'torch-settings-hook');
const SESSION_UPDATE = path.join(ROOT, 'bin', 'torch-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'torch-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'torch-team-test-'));
}
...
describe('torch-settings-hook', () => {
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/torch-session-update`, {
      env: { TORCH_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/torch-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/torch-session-update`, {
      env: { TORCH_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/torch-session-update`, {
      env: { TORCH_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/torch-session-update`, {
      env: { TORCH_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/torch-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/torch-session-update`, {
      env: { TORCH_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('torch-session-update', () => {
  ...
  beforeEach(() => {
    tmpDir = mkTmpDir();
    torchDir = path.join(tmpDir, 'torch');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(torchDir, { recursive: true });
    fs.mkdirSync(stateDir, { recursive: true });

    // Init a git repo to pass the .git guard
    execSync('git init', { cwd: torchDir });
    execSync('git commit --allow-empty -m "init"', { cwd: torchDir });
    fs.writeFileSync(path.join(torchDir, 'VERSION'), '0.1.0');

    // Create a minimal torch-config that returns auto_upgrade=true
    const binDir = path.join(torchDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'torch-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'torch-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(torchDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { TORCH_DIR: torchDir, TORCH_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override torch-config to return false
    fs.writeFileSync(path.join(torchDir, 'bin', 'torch-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { TORCH_DIR: torchDir, TORCH_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { TORCH_DIR: '/nonexistent/path', TORCH_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('torch-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates CLAUDE.md with recommended section', () => {
    ...
    expect(claude).toContain('## torch (recommended)');
  ...
  test('required: creates CLAUDE.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claude).toContain('## torch (REQUIRED');
    expect(claude).toContain('TORCH_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.claude', 'hooks', 'check-torch.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: torch is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-torch');
  });
  ...
  test('idempotent: running twice does not duplicate CLAUDE.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    const matches = claude.match(/## torch/g);
    expect(matches).toHaveLength(1);
  });

  test('removes vendored copy when present', () => {
    // Create a fake vendored torch with VERSION file
    const vendoredDir = path.join(tmpDir, '.claude', 'skills', 'torch');
    ...
    // Track it in git
    execSync('git add .claude/skills/torch/', { cwd: tmpDir });
    execSync('git commit -m "add vendored torch"', { cwd: tmpDir });

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored torch copy');
    ...
    expect(gitignore).toContain('.claude/skills/torch/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored torch copy');
  });

  test('skips when .claude/skills/torch is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'torch'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored torch copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'torch')).isSymbolicLink()).toBe(true);
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.claude\/skills\/torch\//g);
    expect(matches).toHaveLength(1);
  });
});
...
describe('setup --team / --no-team / -q', () => {
  test('setup -q produces no stdout', () => {
    ...
    // The key test is that the "Skill naming:" prompt and "torch ready" messages are suppressed
    expect(result.stdout).not.toContain('Skill naming:');
    expect(result.stdout).not.toContain('torch ready');
  });
