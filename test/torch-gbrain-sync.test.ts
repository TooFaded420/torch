/**
 * Unit tests for bin/torch-gbrain-sync.ts (Lane B).
 *
...
function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "torch-gbrain-sync-"));
}
...
describe("torch-gbrain-sync CLI", () => {
  it("--help exits 0 with usage text", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: torch-gbrain-sync");
  ...
  it("--dry-run with --code-only reports the code import preview only", () => {
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
    ...
    expect(r.stdout).not.toContain("torch-memory-ingest --probe");
    expect(r.stdout).not.toContain("torch-brain-sync --discover-new");
  ...
  it("--dry-run with all stages shows previews for all three", () => {
    ...
    const r = runScript(["--dry-run"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("would: gbrain sources add");
    expect(r.stdout).toContain("gbrain sync --strategy code");
    expect(r.stdout).toContain("would: torch-memory-ingest");
    expect(r.stdout).toContain("would: torch-brain-sync");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("--no-code skips the code import stage", () => {
    ...
    const r = runScript(["--dry-run", "--no-code"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
  ...
  it("dry-run derives a stable source id from the canonical git remote", () => {
    // The source id pattern is `torch-code-<canonicalized-remote>`. For this
    ...
    // that the preview command would target a source with id torch-code-*.
    const home = makeTestHome();
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add torch-code-[a-z0-9-]+/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source torch-code-[a-z0-9-]+/);
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("derived source ids are gbrain-valid (≤32 chars, alnum + interior hyphens, no dots) for any remote", () => {
    ...
    // For `github.com/<org>/<repo>`, the id was `torch-code-github.com-<org>-<repo>`,
    ...
    for (const remote of cases) {
      ...
      const torchHome = join(home, ".torch");
      mkdirSync(torchHome, { recursive: true });
      const repo = mkdtempSync(join(tmpdir(), "torch-source-id-repo-"));
      ...
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, torch_HOME: torchHome },
      });
      ...
      expect(id.startsWith("torch-code-")).toBe(true);
  ...
  it("derives a gbrain-valid source id when the cwd repo has NO origin remote", () => {
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-no-origin-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("derives a gbrain-valid source id when the basename sanitizes to empty", () => {
    ...
    // returned "torch-code-" (invalid trailing hyphen) and was patched to
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const parent = mkdtempSync(join(tmpdir(), "torch-empty-base-"));
  ...
  it("derives distinct source ids for the same absolute path on different hosts", () => {
    ...
    // key keeps them distinct. `torch_HOSTNAME` env var is the test-only knob;
    // production uses `os.hostname()`.
    const home = makeTestHome();
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-host-collide-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    const bindir = mkdtempSync(join(tmpdir(), "torch-host-collide-bin-"));
    ...
    const runAs = (host: string) =>
      spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, torch_HOME: torchHome, torch_HOSTNAME: host, PATH },
      });
  ...
  it("dry-run does NOT acquire the lock file (lock is for write paths only)", () => {
    ...
    const lockPath = join(torchHome, ".sync-gbrain.lock");
  ...
  it("writes a state file with schema_version: 1 after a non-dry run", () => {
    ...
    const statePath = join(torchHome, ".gbrain-sync-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("torch-gbrain-sync");
  ...
  it("records stage results in state file", () => {
    ...
    const state = JSON.parse(readFileSync(join(torchHome, ".gbrain-sync-state.json"), "utf-8"));
  ...
  it("brain-sync stage resolves the sibling binary, not a HOME-rooted path", () => {
    ...
    // ~/.claude/skills/torch/bin/torch-brain-sync, which silently no-op'd
    // on Codex installs and dev workspaces with the misleading summary
    // "skipped (torch-brain-sync not installed)". Post-fix it resolves
    ...
    const r = runScript(
      ...
      { HOME: home, torch_HOME: torchHome },
    );
    ...
    expect(combined).not.toContain("skipped (torch-brain-sync not installed)");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("worktree-aware source ID: two worktrees of the same repo get DIFFERENT ids", () => {
    ...
    // ID was slug-only so both worktrees collapsed onto `torch-code-<slug>` and
    ...
    const repoA = mkdtempSync(join(tmpdir(), "torch-worktree-a-"));
    const repoB = mkdtempSync(join(tmpdir(), "torch-worktree-b-"));
    ...
    expect(idA).not.toBe(idB);
    expect(idA.startsWith("torch-code-")).toBe(true);
    expect(idB.startsWith("torch-code-")).toBe(true);
  ...
  it("worktree-aware source ID: same path produces the same id across runs (deterministic)", () => {
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-worktree-stable-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", remote], { cwd: repo });
  ...
  it("dry-run preview includes legacy-source removal + attach (post-codex-review hardening)", () => {
    // Codex adversarial flagged: pre-pathhash `torch-code-<slug>` sources stay
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-legacy-cleanup-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    // remove the legacy source via `gbrain sources remove torch-code-<slug>
    ...
    expect(r.stdout).toMatch(/gbrain sources add torch-code-/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source torch-code-/);
    expect(r.stdout).toMatch(/gbrain sources attach torch-code-/);
  ...
  it("dry-run preview includes the `sources attach` step (kubectl-style CWD pin)", () => {
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-attach-preview-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/garrytan/gstack.git"], { cwd: repo });
...
// creating a data-loss window. See bin/torch-gbrain-sync.ts and the
...
describe("derivePathOnlyHashLegacyId", () => {
  it("returns the pre-#1468 form (path-only sha1, no hostname)", () => {
    ...
    // legacy id regardless of $torch_HOSTNAME, because the pre-#1468 hash
    // didn't include hostname.
    const repo = mkdtempSync(join(tmpdir(), "torch-legacy-id-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      ...
      process.env.torch_HOSTNAME = "machine-a";
      const b = derivePathOnlyHashLegacyId(repo);
      process.env.torch_HOSTNAME = "machine-b";
      const c = derivePathOnlyHashLegacyId(repo);
      expect(a).toBe(b);
      expect(b).toBe(c);
      expect(a.startsWith("torch-code-")).toBe(true);
      expect(a.length).toBeLessThanOrEqual(32);
    } finally {
      delete process.env.torch_HOSTNAME;
      process.chdir(cwd);
      rmSync(repo, { recursive: true, force: true });
    }
  });
  ...
  it("produces a different id than the new hostname-folded form", () => {
    ...
    const repo = mkdtempSync(join(tmpdir(), "torch-legacy-id-distinct-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      process.chdir(repo);
      process.env.torch_HOSTNAME = "machine-x";
      const legacy = derivePathOnlyHashLegacyId(repo);
      // Drive the new id through the CLI so we use the same code path users hit.
      const home = makeTestHome();
      const torchHome = join(home, ".torch");
      mkdirSync(torchHome, { recursive: true });
      const bindir = mkdtempSync(join(tmpdir(), "torch-legacy-id-distinct-bin-"));
      makeShim(bindir, { "--help": { stdout: "gbrain\n" } });
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        encoding: "utf-8",
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, torch_HOME: torchHome, torch_HOSTNAME: "machine-x", PATH: `${bindir}:${process.env.PATH || ""}` },
      });
...
describe("planHostnameFoldMigration", () => {
  ...
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "torch-mig-plan-bin-"));
    _resetGbrainSupportsRenameCache();
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  ...
  it("returns ids-match when legacy == new (degenerate case)", () => {
    const result = planHostnameFoldMigration("/repo/path", "torch-code-same-abc12345", "torch-code-same-abc12345");
...
describe("constrainSourceId truncation (hyphen-boundary cut)", () => {
  ...
  //
  // Exercised via the dry-run preview (`gbrain sources add torch-code-…`),
  // since constrainSourceId is module-private.
  it("never produces mid-word truncation artifacts like `kill` (from `skill`)", () => {
    const home = makeTestHome();
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-hyphen-cut-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("produces a period-free source id for HTTPS remotes ending in .git (#1357)", () => {
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "torch-https-period-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
...
describe("sourceLocalPath", () => {
  let bindir: string;
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "torch-source-lp-bin-"));
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  });
