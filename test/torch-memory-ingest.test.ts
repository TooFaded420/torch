/**
 * Unit tests for bin/torch-memory-ingest.ts (Lane A).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "torch-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "torch-memory-ingest-"));
}
...
describe("torch-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: torch-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds torch artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const torchHome = join(home, ".torch");
    mkdirSync(join(torchHome, "analytics"), { recursive: true });
    mkdirSync(join(torchHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(torchHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(torchHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(torchHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const torchHome = join(home, ".torch");
    mkdirSync(join(torchHome, "analytics"), { recursive: true });
    mkdirSync(join(torchHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(torchHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(torchHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("torch-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const torchHome = join(home, ".torch");
    mkdirSync(torchHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, torch_HOME: torchHome });
    expect(r.exitCode).toBe(0);
...
describe("torch-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "torch-mi-cwd-marker-"));
    ...
    const r = runScript(["--incremental", "--quiet"], {
      ...
      torch_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Claude Code JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "torch-mi-parse-"));
    ...
    const r = runScript(["--probe"], { HOME: home, torch_HOME: join(home, ".torch") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
...
describe("torch-memory-ingest --limit", () => {
...
describe("torch-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
