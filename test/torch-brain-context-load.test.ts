/**
 * Unit tests for bin/torch-brain-context-load.ts (Lane C).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "torch-brain-context-load.ts");
...
describe("torch-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: torch-brain-context-load");
...
describe("torch-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "torch-bcl-"));
...
describe("torch-brain-context-load — graceful gbrain absence", () => {
