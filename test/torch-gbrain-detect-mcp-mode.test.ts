/**
 * torch-gbrain-detect — gbrain_mcp_mode + torch_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'torch-gbrain-detect');
...
describe('torch_artifacts_remote', () => {
  test('reads ~/.torch-artifacts-remote.txt when present', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.torch-artifacts-remote.txt'),
      'https://github.com/garrytan/gstack-artifacts-garrytan\n'
    );
    expect(runDetect().json.torch_artifacts_remote).toBe(
      'https://github.com/garrytan/gstack-artifacts-garrytan'
    );
  });

  test('migration-window fallback: reads ~/.torch-brain-remote.txt if artifacts file is missing', () => {
    fs.writeFileSync(
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.torch_artifacts_remote).toBe('https://github.com/x/new');
  });
...
describe('schema regression', () => {
  test('output JSON has all expected keys (sync-gbrain compat)', () => {
    ...
    expect(keys).toEqual([
      ...
      'gbrain_pooler_mode',
      'gbrain_version',
      'torch_artifacts_remote',
      'torch_brain_git',
      'torch_brain_sync_mode',
    ]);
  });
});
