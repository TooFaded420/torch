/**
 * torch-upgrade/migrations/v1.40.0.0.sh — migration script unit tests.
 *
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "torch-mig-v1400-"));
  torchHome = path.join(tmpHome, ".torch");
  migrationDir = path.join(torchHome, ".migrations");
  donePath = path.join(migrationDir, "v1.40.0.0.done");
  allowlistPath = path.join(torchHome, ".brain-allowlist");
  privacyPath = path.join(torchHome, ".brain-privacy-map.json");
  gitattrsPath = path.join(torchHome, ".gitattributes");
  fs.mkdirSync(torchHome, { recursive: true });
});

afterEach(() => {
  try {
    fs.chmodSync(torchHome, 0o755);
...
function makeCuratedPath(opts: { jq?: "missing" | "shim-fail" | "real" } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "torch-mig-path-"));
  const utils = [
    "bash",
    "sh",
    "mkdir",
