/**
 * v1.27.0.0 migration — torch-brain → torch-artifacts rename.
 *
...
const MIGRATION = path.join(ROOT, 'torch-upgrade', 'migrations', 'v1.27.0.0.sh');
  ...
  repo)
    ...
    case "$1" in
      view)
        # gh repo view <name>
        shift
        ${alreadyRenamed ? `if echo "$@" | grep -q torch-artifacts; then exit 0; else exit 1; fi` : `exit 1`}
        ;;
...
case "$1 $2" in
  "sources list")
    ${hasOld ? `echo "torch-brain-testuser ~/.torch-brain-worktree"` : 'true'}
    exit 0
    ;;
...
beforeEach(() => {
  ...
  fs.mkdirSync(path.join(tmpHome, '.torch'), { recursive: true });
});
...
describe('v1.27.0.0 migration — nothing to migrate', () => {
  test('no legacy state → exits 0, writes done touchfile, no journal', () => {
    // Fresh HOME: no brain-remote.txt, no .torch/.git
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('nothing to migrate');
    expect(fs.existsSync(path.join(tmpHome, '.torch/.migrations/v1.27.0.0.done'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.torch/.migrations/v1.27.0.0.journal'))).toBe(false);
  });

  test('done touchfile present → exits 0 silently (no re-prompt)', () => {
    fs.mkdirSync(path.join(tmpHome, '.torch/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.torch/.migrations/v1.27.0.0.done'), '');
    const r = run();
    expect(r.code).toBe(0);
  ...
  test('skipped-by-user touchfile → exits 0 silently', () => {
    fs.mkdirSync(path.join(tmpHome, '.torch/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.torch/.migrations/v1.27.0.0.skipped-by-user'), '');
    fs.writeFileSync(path.join(tmpHome, '.torch-brain-remote.txt'), 'https://github.com/x/torch-brain-testuser');
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toBe('');
  });
...
describe('v1.27.0.0 migration — GitHub host (non-interactive)', () => {
  beforeEach(() => {
    fs.writeFileSync(
      ...
      'https://github.com/testuser/torch-brain-testuser\n'
    );
    fs.writeFileSync(
      path.join(tmpHome, '.torch/config.yaml'),
  ...
  test('renames repo, mvs remote.txt, rewrites config key, writes done', () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, '.torch-brain-remote.txt'))).toBe(false);
    const newUrl = fs.readFileSync(path.join(tmpHome, '.torch-artifacts-remote.txt'), 'utf-8').trim();
    expect(newUrl).toBe('https://github.com/testuser/torch-artifacts-testuser');
    // Config key renamed.
    const cfg = fs.readFileSync(path.join(tmpHome, '.torch/config.yaml'), 'utf-8');
...
describe('v1.27.0.0 migration — interruption resume', () => {
  ...
  test('partial journal: skips already-done steps', () => {
    ...
    const migDir = path.join(tmpHome, '.torch/.migrations');
...
describe('v1.27.0.0 migration — local CLI sources swap (codex Finding #6 ordering)', () => {
  test('add-new before remove-old (verify by call order in log)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.torch/.git'), { recursive: true }); // brain repo present
    makeFakeGh({});
    ...
    const log = fs.readFileSync(path.join(fakeBinDir, 'gbrain-calls.log'), 'utf-8');
    const addIdx = log.indexOf('gbrain sources add torch-artifacts-testuser');
    const removeIdx = log.indexOf('gbrain sources remove torch-brain-testuser');
  ...
  test('add fails → old source stays registered (no silent loss)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.torch/.git'), { recursive: true });
    makeFakeGh({});
