/**
 * Subprocess tests for bin/torch-decision-log + bin/torch-decision-search.
 * Mirrors the learnings-bins test pattern (run the bin with torch_HOME=tmp).
 */
...
const LOG = path.join(ROOT, "bin", "torch-decision-log");
const SEARCH = path.join(ROOT, "bin", "torch-decision-search");

let tmpDir: string;

function opts(): ExecSyncOptionsWithStringEncoding {
  return { cwd: ROOT, env: { ...process.env, torch_HOME: tmpDir }, encoding: "utf-8", timeout: 20000 };
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "torch-decision-"));
  fs.mkdirSync(path.join(tmpDir, "projects"), { recursive: true });
});
afterEach(() => fs.rmSync(tmpDir, { recursive: true, force: true }));

describe("torch-decision-log", () => {
...
describe("torch-decision-search", () => {
  ...
  test("redact + compact expunges everywhere", () => {
    ...
    const archive = path.join(tmpDir, "projects", "garrytan-torch", "decisions.archive.jsonl");
...
describe("torch-decision-search --semantic (optional gbrain enhancement)", () => {
  ...
  function searchWithPath(args: string, pathPrefix?: string): string {
    const env = { ...process.env, torch_HOME: tmpDir } as NodeJS.ProcessEnv;
  ...
  test("--semantic --query appends a related-memory block when gbrain returns hits", () => {
    ...
    const dir = shimDir(
      `#!/usr/bin/env bash
if [ "$1" = "sources" ]; then echo '{"sources":[{"id":"default","local_path":"/u/.torch-brain-worktree"}]}'; exit 0; fi
...
describe("torch-decision-search --recent / --scope / datamark", () => {
