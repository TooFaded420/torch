...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // torch-review-read reads from ~/.torch/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock torch-review-read script that returns our test data
    ...
    // Write a mock torch-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'torch-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'torch-review-read'), 0o755);
...
Instead of running ~/.claude/skills/torch/bin/torch-review-read, run this mock: ${mockBinDir}/torch-review-read
