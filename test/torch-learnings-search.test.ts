...
const BIN = path.join(ROOT, 'bin', 'torch-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-search-cwd-'));
// torch-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('torch-learnings-search token-OR query semantics', () => {
...
describe('torch-learnings-search cross-project trust gating', () => {
