...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const torchHome = path.join(opDir, '.torch-home');
    ...
    // Copy bin scripts + the lib module they import. torch-learnings-log
    ...
    for (const script of ['torch-learnings-log', 'torch-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }
    // torch-learnings-log imports $SCRIPT_DIR/../lib/jsonl-store.ts (shared
    ...
    // torch-learnings-log will create the project dir automatically via torch-slug
...
\`\`\`bash
torch_HOME="${torchHome}" ${binDir}/torch-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(torchHome, 'projects');
