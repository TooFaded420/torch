...
const BIN = path.join(ROOT, 'bin', 'torch-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/torch/bin/torch-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`torch-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('torch-paths', () => {
  test('torch_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      torch_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.torch_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA ignored when CLAUDE_PLUGIN_ROOT is absent or non-torch', () => {
    // Without CLAUDE_PLUGIN_ROOT, falls through to HOME path.
    const noRoot = run({ CLAUDE_PLUGIN_DATA: '/tmp/plugin-data', HOME: '/tmp/home' });
    expect(noRoot.torch_STATE_ROOT).toBe('/tmp/home/.torch');

    // With a CLAUDE_PLUGIN_ROOT that doesn't contain "torch" (e.g. the codex plugin),
    ...
    expect(wrongRoot.torch_STATE_ROOT).toBe('/tmp/home/.torch');
  });

  test('CLAUDE_PLUGIN_DATA respected when CLAUDE_PLUGIN_ROOT identifies torch', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/torch-plugin-data',
      CLAUDE_PLUGIN_ROOT: '/tmp/torch-garrytan',
      HOME: '/tmp/home',
    });
    expect(got.torch_STATE_ROOT).toBe('/tmp/torch-plugin-data');
  });

  test('HOME-derived state root when torch_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.torch_STATE_ROOT).toBe('/tmp/myhome/.torch');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.torch_STATE_ROOT).toBe('.torch');
  });

  test('PLAN_ROOT chain: torch_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ torch_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
