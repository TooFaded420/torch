/**
 * Project-slug consistency for torch-repo-mode.
 *
 * These are end-to-end reproductions of #2212. Each case is a real local Git
 * workspace which predates its public GitHub origin: torch-slug caches the
...
const REPO_MODE_BIN = path.join(ROOT, 'bin', 'torch-repo-mode');
...
function run(command: string, args: string[], cwd: string, home: string): CommandResult {
  const result = spawnSync(command, args, {
    cwd,
    encoding: 'utf8',
    env: { ...process.env, HOME: home, torch_HOME: path.join(home, '.torch') },
  });
  return {
...
describe('torch-repo-mode cached slug consistency (#2212)', () => {
  for (const workspace of publicWorkspaces) {
    test(`${workspace.origin} keeps its pre-origin project cache`, () => {
      ...
      try {
        ...
        git(['config', 'user.name', 'torch regression test'], project, home);
        git(['config', 'user.email', 'test@torch.dev'], project, home);

        // This workspace existed and used torch before it adopted GitHub.
        ...
        const projectsDir = path.join(home, '.torch', 'projects');
  ...
  test('still treats a workspace without origin as unknown', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(home, '.torch', 'projects'))).toBe(false);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('sanitizes unusual origin URLs via torch-slug before mkdir', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'grepo-home-'));
    ...
    try {
      ...
      const projects = fs.readdirSync(path.join(home, '.torch', 'projects'));
