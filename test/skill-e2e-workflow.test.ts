...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-ship-work-'));
...
describeIfSelected('torch-upgrade E2E', ['torch-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('torch-upgrade-happy-path', async () => {
    const mockTorch = path.join(upgradeDir, '.claude', 'skills', 'torch');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/torch — use that exact path.`,
    ...
    recordE2E(evalCollector, '/torch-upgrade happy path', 'torch-upgrade E2E', result, {
