...
const MIGRATION = path.join(ROOT, 'torch-upgrade', 'migrations', 'v1.1.3.0.sh');
...
function setupFaketorchRoot(tmpHome: string): string {
  // A real target that the torch symlink can resolve into.
  const torchDir = path.join(tmpHome, '.claude', 'skills', 'torch');
  fs.mkdirSync(path.join(torchDir, 'checkpoint'), { recursive: true });
  fs.writeFileSync(path.join(torchDir, 'checkpoint', 'SKILL.md'), '# fake torch checkpoint\n');
  return torchDir;
}
...
describe('migration v1.1.3.0 — checkpoint ownership guard', () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-migration-ownership-'));
  });
  ...
  test('scenario A: directory symlink into torch → removed', () => {
    setupFaketorchRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const torchCheckpoint = path.join(skillsDir, 'torch', 'checkpoint');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.symlinkSync(torchCheckpoint, topLevel);

    const result = runMigration(tmpHome);
    expect(result.exitCode).toBe(0);
    expect(fs.existsSync(topLevel)).toBe(false);
    // Also removes the torch-owned inner copy (Shape 2 cleanup).
    expect(fs.existsSync(torchCheckpoint)).toBe(false);
    expect(result.stdout).toContain('Removed stale /checkpoint symlink');
  });

  test('scenario B: directory with SKILL.md symlinked into torch → removed', () => {
    setupFaketorchRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const torchSKILL = path.join(skillsDir, 'torch', 'checkpoint', 'SKILL.md');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    fs.symlinkSync(torchSKILL, path.join(topLevel, 'SKILL.md'));
  ...
  test('scenario C: user-owned regular directory with custom content → preserved', () => {
    ...
    expect(result.stdout).toContain('not a torch-owned install');
  });

  test('scenario D: symlink pointing outside torch → preserved', () => {
    setupFaketorchRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    ...
    expect(result.stdout).toContain('Leaving');
    expect(result.stdout).toContain('outside torch');
  });

  test('scenario E: nothing to do → no-op exit 0 (idempotent)', () => {
    // No checkpoint install at all. First run: nothing removed.
    setupFaketorchRoot(tmpHome);
    // Delete the inner torch/checkpoint to simulate post-upgrade state.
    fs.rmSync(path.join(tmpHome, '.claude', 'skills', 'torch', 'checkpoint'), { recursive: true, force: true });
  ...
  test('scenario F: torch not installed → no-op exit 0', () => {
    // No ~/.claude/skills/torch/ at all. Also no checkpoint install.
  ...
  test('scenario G: SKILL.md is a symlink pointing outside torch → preserved', () => {
    setupFaketorchRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    // A directory containing SKILL.md that's a symlink pointing outside torch.
