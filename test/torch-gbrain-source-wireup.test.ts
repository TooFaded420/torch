/**
 * torch-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the torch brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'torch-gbrain-source-wireup');

let tmpHome: string;
let torchHome: string;
...
function run(
  ...
  const env = {
    ...
    torch_BRAIN_WORKTREE: worktreeDir,
    torch_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setuptorchRepo(remoteUrl: string) {
  // Real git repo at torchHome with at least one commit + an origin remote.
  fs.mkdirSync(torchHome, { recursive: true });
  spawnSync('git', ['-C', torchHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', torchHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', torchHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(torchHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', torchHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', torchHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', torchHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-wireup-test-'));
  torchHome = path.join(tmpHome, '.torch');
  worktreeDir = path.join(tmpHome, '.torch-brain-worktree');
...
describe('torch-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setuptorchRepo('git@github.com:user/torch-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { torch_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('torch-brain-user');
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'torch-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { torch_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove torch-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add torch-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('source-id derived from origin URL', () => {
    setuptorchRepo('git@github.com:user/torch-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { torch_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('torch-brain-alice');
  });

  test('source-id fallback to ~/.torch-brain-remote.txt when .git is gone', () => {
    // No git repo at torchHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.torch-brain-remote.txt'),
      'git@github.com:user/torch-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .torch/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setuptorchRepo('git@github.com:user/torch-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { torch_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=torch-brain-user');
...
describe('torch-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { torch_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('torch-gbrain-source-wireup — uninstall mode', () => {
...
describe('torch-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(torchHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', torchHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', torchHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', torchHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { torch_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
