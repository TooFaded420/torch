...
describe("torch-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.TORCH_SEARCH_SLUG");
    expect(bunBlock).toContain("process.env.TORCH_SEARCH_CROSS");
  });
  ...
  test("env vars are set on the bun command line", () => {
    ...
    expect(script).toContain("TORCH_SEARCH_SLUG=");
