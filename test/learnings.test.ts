...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, TORCH_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 15000,
  };
  try {
    const stdout = execSync(`${BIN}/torch-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/torch-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-learn-'));
...
describe('torch-learnings-search', () => {
...
describe('torch-learnings-log edge cases', () => {
...
describe('torch-learnings-search edge cases', () => {
