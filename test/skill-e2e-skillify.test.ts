 ...
 *      Skill written to ~/.torch/browser-skills/<name>/ with full
 ...
 *   - A scoped torch_HOME under the workdir so on-disk artifacts are
...
function setupSkillifyWorkdir(suffix: string, installSkills: string[] = ['scrape', 'skillify']): Workdir {
  ...
  for (const script of [
    'torch-timeline-log', 'torch-slug', 'torch-config',
    'torch-update-check', 'torch-repo-mode',
 ...
 * has to live under <workdir>/.torch/browser-skills/ for the project-tier
 * lookup to find it (torch's bundled tier resolves from the install dir,
...
function installBundledHackernewsSkill(workDir: string) {
  ...
  const dst = path.join(workDir, '.torch', 'browser-skills', 'hackernews-frontpage');
  copyDirSync(src, dst);
}
...
describeIfSelected('Browser-skills Phase 2a E2E (/scrape + /skillify)', [
  ...
  testConcurrentIfSelected('scrape-match-path', async () => {
    const { workDir, torchHome } = setupSkillifyWorkdir('match', ['scrape']);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('scrape-prototype-path', async () => {
    const { workDir, torchHome } = setupSkillifyWorkdir('prototype', ['scrape']);
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('skillify-happy-path', async () => {
    const { workDir, torchHome } = setupSkillifyWorkdir('happy', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
(translates to ~/.torch/browser-skills/<name>/ via $HOME).

Do NOT halt for clarification.`,
      workingDirectory: workDir,
      env: {
        torch_HOME: torchHome,
        HOME: workDir,  // /skillify writes to $HOME/.torch/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill should land in $HOME/.torch/browser-skills/<name>/
    const skillsRoot = path.join(workDir, '.torch', 'browser-skills');
  ...
  testConcurrentIfSelected('skillify-provenance-refusal', async () => {
    const { workDir, torchHome } = setupSkillifyWorkdir('refusal', ['skillify']);
...
write any files.`,
    ...
    const skillsRoot = path.join(workDir, '.torch', 'browser-skills');
    const stagingRoot = path.join(workDir, '.torch', '.tmp');
  ...
  testConcurrentIfSelected('skillify-approval-reject', async () => {
    const { workDir, torchHome } = setupSkillifyWorkdir('reject', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
