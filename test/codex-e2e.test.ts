...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/torch-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install torch-review skill to a temp HOME and ask Codex to list skills
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'torch-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('torch') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the torch-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install torch-review skill and ask Codex to review the worktree
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'torch-review');

    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the torch-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'torch-review',
    });
