...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['torch-learnings-search', 'torch-learnings-log', 'torch-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what torch-slug computes.
    // With no git remote, torch-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/torch/bin/torch-slug with ./bin/torch-slug.
