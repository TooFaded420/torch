/**
 * torch-settings-hook schema-aware surface (T3 plan-tune cathedral).
 *
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-shsa-'));
...
function run(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: { ...process.env, torch_SETTINGS_FILE: settingsFile },
      encoding: 'utf-8',
      timeout: 10000,
    });
...
describe('add-event', () => {
  test('registers a PreToolUse hook with matcher + source tag', () => {
    ...
    expect(s.hooks.PreToolUse[0]._torch_source).toBe('plan-tune-cathedral');
