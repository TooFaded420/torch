...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
# Vendoring deprecation: detect if CWD has a vendored torch copy
_VENDORED="no"
...
If `PROACTIVE` is `"false"`, do not proactively suggest torch skills AND do not
...
or invoking other torch skills, use the `/torch-` prefix (e.g., `/torch-qa` instead
of `/qa`, `/torch-ship` instead of `/ship`). Disk paths are unaffected — always use
`$TORCH_ROOT/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$TORCH_ROOT/torch-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running torch v{to} (just updated!)" and continue.

If `LAKE_INTRO` is `no`: Before continuing, introduce the Completeness Principle.
Tell the user: "torch follows the **Boil the Lake** principle — always do the complete
...
> Help torch get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `torch-config set telemetry off`.
...
> How about anonymous mode? We just learn that *someone* used torch — no unique ID,
...
> torch can proactively figure out when you might need a skill while you work —
...
If B: run `$TORCH_BIN/torch-config set routing_declined true`
Say "No problem. You can add routing rules later by running `torch-config set routing_declined false` and re-running any skill."

This only happens once per project. If `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`, skip this entirely.

If `VENDORED_TORCH` is `yes`: This project has a vendored copy of torch at
`.factory/skills/torch/`. Vendoring is deprecated. We will not keep vendored copies
up to date, so this project's torch will fall behind.

Use AskUserQuestion (one-time per project, check for `~/.torch/.vendoring-warned-$SLUG` marker):
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+torch. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.
...
`~/.torch/analytics/` (user config directory, not project files). The skill
...
- Writing to `~/.torch/` (config, analytics, review logs, design artifacts, learnings)
...
1. Check if the plan file already has a `## TORCH REVIEW REPORT` section.
...
Then write a `## TORCH REVIEW REPORT` section to the end of the plan file:
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $TORCH_BIN/torch-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $TORCH_BIN/torch-learnings-search --limit 10 2>/dev/null || true
fi
```
...
OLD_CFG=$($TORCH_ROOT/bin/torch-config get codex_reviews 2>/dev/null || true)
