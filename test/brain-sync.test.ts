 ...
 *   - bin/torch-config gbrain keys (validation, isolation)
 *   - bin/torch-brain-enqueue (atomicity, skip list, no-op gates)
 *   - bin/torch-jsonl-merge (3-way, ts-sort, hash-fallback)
 *   - bin/torch-brain-sync --once (drain, commit, push, secret-scan, skip-file)
 *   - bin/torch-artifacts-init + --restore round-trip
 *   - bin/torch-brain-uninstall preserves user data
 *   - env isolation (torch_HOME never bleeds into real ~/.torch/config.yaml)
 *
 * Runs each test against a temp torch_HOME and a local bare git repo as
...
function run(argv: string[], opts: { env?: Record<string, string>; input?: string } = {}) {
  ...
  const res = spawnSync(full, argv.slice(1), {
    env: { ...process.env, torch_HOME: tmpHome, ...(opts.env || {}) },
    encoding: 'utf-8',
    input: opts.input,
    cwd: ROOT,
  });
...
afterEach(() => {
  ...
  const remoteFile = path.join(os.homedir(), '.torch-brain-remote.txt');
...
describe('torch-config gbrain keys', () => {
  ...
  test('torch_HOME overrides real config dir', () => {
    ...
    // Real ~/.torch/config.yaml must not be touched.
...
// Enqueue behavior
// ---------------------------------------------------------------
describe('torch-brain-enqueue', () => {
  test('no-op when feature not initialized', () => {
    const r = run(['torch-brain-enqueue', 'projects/foo/learnings.jsonl']);
    expect(r.status).toBe(0);
  ...
  test('enqueues when mode is full and .git exists', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'), { recursive: true });
    run(['torch-config', 'set', 'artifacts_sync_mode', 'full']);
    run(['torch-brain-enqueue', 'projects/foo/learnings.jsonl']);
  ...
  test('skip list honored', () => {
    ...
    run(['torch-brain-enqueue', 'projects/foo/secret.jsonl']);
    run(['torch-brain-enqueue', 'projects/foo/ok.jsonl']);
    const queue = fs.readFileSync(path.join(tmpHome, '.brain-queue.jsonl'), 'utf-8');
  ...
  test('concurrent enqueues all land (atomic append)', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      procs.push(new Promise<void>((resolve) => {
        const r = spawnSync(path.join(BIN, 'torch-brain-enqueue'), [`file-${i}.jsonl`], {
  ...
  test('no args does not crash', () => {
    const r = run(['torch-brain-enqueue']);
    expect(r.status).toBe(0);
  });
});
...
describe('torch-jsonl-merge', () => {
  test('3-way merge dedups + sorts by ts', () => {
    ...
    const r = run([path.join(BIN, 'torch-jsonl-merge'), base, ours, theirs]);
    expect(r.status).toBe(0);
  ...
  test('falls back to hash order for lines without ts', () => {
    ...
    run([path.join(BIN, 'torch-jsonl-merge'), base, ours, theirs]);
    const lines = fs.readFileSync(ours, 'utf-8').trim().split('\n');
    expect(lines.length).toBe(3);
    // Order is deterministic (sha256 of each line).
    const again = spawnSync(path.join(BIN, 'torch-jsonl-merge'), [base, ours, theirs]);
...
describe('init + sync + restore round-trip', () => {
  test('init creates canonical files + registers drivers', () => {
    const r = run(['torch-artifacts-init', '--remote', bareRemote]);
    expect(r.status).toBe(0);
  ...
  test('full sync: init → enqueue → --once → commit pushed', () => {
    ...
    run(['torch-brain-enqueue', 'projects/p/learnings.jsonl']);
    const r = run(['torch-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('restore round-trip: writes on machine A visible on machine B', () => {
    ...
    run(['torch-brain-enqueue', 'projects/myproj/learnings.jsonl']);
    run(['torch-brain-sync', '--once']);

    // Machine B (new temp home).
    const machineB = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-machineB-'));
    const r = run(['torch-brain-restore', bareRemote], {
      env: { torch_HOME: machineB },
    });
    expect(r.status).toBe(0);
...
// ---------------------------------------------------------------
describe('torch-brain-sync secret scan', () => {
  ...
  test('--skip-file unblocks specific file', () => {
    ...
    run(['torch-brain-enqueue', leakPath]);
    run(['torch-brain-sync', '--once']);  // blocked
    run(['torch-brain-sync', '--skip-file', leakPath]);
...
// ---------------------------------------------------------------
describe('torch-brain-uninstall', () => {
  test('removes sync config but preserves learnings/project data', () => {
    ...
    const r = run(['torch-brain-uninstall', '--yes']);
    expect(r.status).toBe(0);
    ...
    // Config key reset.
    const mode = run(['torch-config', 'get', 'artifacts_sync_mode']);
...
// ---------------------------------------------------------------
describe('torch-brain-sync --discover-new', () => {
  test('enqueues new allowlisted files; idempotent on re-run', () => {
    ...
    run(['torch-brain-sync', '--discover-new']);
