...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    // torch-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(torchHome);
...
IMPORTANT:
- Use torch_HOME="${torchHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/torch/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/torch/scripts/ references with ./scripts/.
