...
describe('validateHostConfig', () => {
  function makeValid(): HostConfig {
    return {
      ...
      cliCommand: 'testcli',
      globalRoot: '.test/skills/torch',
      localSkillRoot: '.test/skills/torch',
      hostSubdir: '.test',
      usesEnvVars: true,
  ...
  test('paths with $ and ~ are valid', () => {
    const c = makeValid();
    c.globalRoot = '$HOME/.test/skills/torch';
    c.localSkillRoot = '~/.test/skills/torch';
    expect(validateHostConfig(c)).toEqual([]);
  });
...
describe('validateAllConfigs', () => {
  ...
  test('duplicate hostSubdir detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.claude', globalRoot: '.dup/skills/torch' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
    expect(errors.some(e => e.includes('Duplicate hostSubdir'))).toBe(true);
  });

  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.claude/skills/torch' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
  ...
  test('unknown suppressedResolvers entry surfaces with host-name prefix', () => {
    const bad = { ...codex, name: 'bad-host', hostSubdir: '.bad', globalRoot: '.bad/skills/torch', suppressedResolvers: ['BOGUS_RESOLVER'] } as HostConfig;
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.claude/skills/torch');
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/torch/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/torch/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/torch/design/dist');
  });

  test('Codex uses $torch_ROOT env vars', () => {
    expect(HOST_PATHS.codex.skillRoot).toBe('$torch_ROOT');
    expect(HOST_PATHS.codex.binDir).toBe('$torch_BIN');
    expect(HOST_PATHS.codex.browseDir).toBe('$torch_BROWSE');
    expect(HOST_PATHS.codex.designDir).toBe('$torch_DESIGN');
  });

  test('every host with usesEnvVars=true gets env var paths', () => {
    for (const config of ALL_HOST_CONFIGS) {
      if (config.usesEnvVars) {
        expect(HOST_PATHS[config.name].skillRoot).toBe('$torch_ROOT');
        expect(HOST_PATHS[config.name].binDir).toBe('$torch_BIN');
      }
    }
  });
...
describe('host-config-export.ts CLI', () => {
  ...
  test('get returns string field', () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toBe('.codex/skills/torch');
  });
...
describe('golden-file regression', () => {
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'torch-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.factory', 'skills', 'torch-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});
...
describe('host config correctness', () => {
  ...
  test('codex has sidecar config', () => {
    ...
    expect(codex.sidecar!.path).toBe('.agents/skills/torch');
  });
