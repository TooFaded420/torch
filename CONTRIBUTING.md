...
Thanks for wanting to make torch better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

torch skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/torch/` (your global install). But when you're developing torch itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone https://github.com/garrytan/gstack.git && cd torch
...
torch automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.torch/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so torch gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use torch normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.torch/projects/*/learnings.jsonl`
3. **Fork and clone torch** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where torch annoyed you)
   ln -sfn /path/to/your/torch-fork .claude/skills/torch
   cd .claude/skills/torch && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> torch/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using torch** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix torch while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ torch sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on torch inside the torch repo

When you're editing torch skills and want to test them by actually using torch
...
```
torch/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── torch -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> torch/review/SKILL.md
│   ├── ship/                    <- or torch-review/, torch-ship/ if --prefix
│   │   └── SKILL.md -> torch/ship/SKILL.md
...
under `torch/`. Names depend on your prefix setting (`~/.torch/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/torch-review`, `/torch-ship`).
...
If gbrain is installed and usable (`bin/torch-gbrain-detect --is-ok` exits 0),
...
into `.claude/torch-rendered/` (gitignored, per-workspace). It then repoints the
...
passes `torch_SKIP_GBRAIN_REGEN=1` inline to the nested `./setup` (so it never
dirties tracked source) and runs `gen:skill-docs:user --out-dir .claude/torch-rendered`,
...
sessions, run `torch-config gbrain-refresh`, which renders them into the global
install (`~/.claude/skills/torch`), guarded so it never touches a symlinked or
non-torch directory.
...
seeded `CLAUDE_CONFIG_DIR`, a temp `torch_HOME`, and `--strict-mcp-config`. Your
operator `~/.claude` config, MCP servers (gbrain, Conductor), skills, `~/.torch`
...
When E2E tests run, they produce machine-readable artifacts in `~/.torch-dev/`:
...
wrap the eval command in `bin/torch-detach`: a fresh session that escapes a
turn-boundary SIGTERM, a `caffeinate` wrapper that blocks idle-sleep, a machine-wide
`torch-evals` lock so concurrent worktrees serialize instead of saturating the
model API, a run-scoped log under `~/.torch-dev/eval-runs/`, a per-tier watchdog,
and a guaranteed `### torch-detach EXIT=<code> ###` sentinel so a poller never
...
Artifacts are never cleaned up — they accumulate in `~/.torch-dev/` for post-mortem debugging and trend analysis.
...
torch's Writing Style section (injected into every tier-≥2 skill's preamble)
...
torch generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/torch-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/torch` vs `$torch_ROOT` |
...
|------|--------|-------------|
| `setup` | `bin/dev-setup` | Copies `.env` from main worktree, installs deps, symlinks skills, runs `./setup` non-interactively, and (if gbrain is installed) renders brain-aware blocks into `.claude/torch-rendered/` without dirtying tracked source |
| `archive` | `bin/dev-teardown` | Removes skill symlinks, the `.claude/torch-rendered/` render, and cleans up `.claude/` directory |

When Conductor creates a new workspace, `bin/dev-setup` runs automatically. It detects the main worktree (via `git worktree list`), copies your `.env` so API keys carry over, and sets up dev mode — no manual steps needed.

`bin/dev-setup` runs `./setup` fully non-interactively (it passes `--plan-tune-hooks=prompt` and closes stdin), so a forwarded Conductor TTY can never hang on a hidden setup prompt. It also never installs the plan-tune Claude Code hooks, which means a throwaway workspace can't rewrite your global `~/.claude/settings.json` to point at an ephemeral worktree path. To install the plan-tune hooks deliberately, run `./setup --plan-tune-hooks` outside dev-setup (or `torch-config set plan_tune_hooks yes`).

**First-time setup:** Put your `ANTHROPIC_API_KEY` in `.env` in the main repo (see `.env.example`). Every Conductor workspace inherits it automatically.

**`torch_*` env prefix (Conductor-injected keys).** Conductor explicitly strips `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` from every workspace's process env. The `.env` copy path doesn't restore them either — the strip happens after env inheritance. Users who want paid evals, `/sync-gbrain` embeddings, or `claude-agent-sdk` calls to work in a Conductor workspace must set `torch_ANTHROPIC_API_KEY` and `torch_OPENAI_API_KEY` in Conductor's workspace env config; Conductor passes those through untouched. On the torch side, TS entry points import `lib/conductor-env-shim.ts` as a side effect, which promotes `torch_FOO_API_KEY` to `FOO_API_KEY` when the canonical name is empty. If you add a new TS entry point that hits a paid API, add `import "../lib/conductor-env-shim";` to the top of the file. Today the shim is imported from `bin/torch-gbrain-sync.ts`, `bin/torch-model-benchmark`, `scripts/preflight-agent-sdk.ts`, and `test/helpers/e2e-helpers.ts`.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/torch`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop torch.** Symlink your torch checkout
...
```bash
# In your core project (not the torch repo)
ln -sfn /path/to/your/torch-checkout .claude/skills/torch
```

### Step 2: Run setup to create per-skill symlinks

The `torch` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `torch/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/torch && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/torch-qa`).
Your choice is saved to `~/.torch/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/torch/`:

```bash
rm .claude/skills/torch
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `torch/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed torch with one prefix setting and want to switch:

```bash
cd .claude/skills/torch && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/torch && ./setup --prefix       # switch to /torch-qa, /torch-ship
```
...
- Renamed or moved config keys in `~/.torch/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.torch/` state files
...
1. Create `torch-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x torch-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/torch-relink" 2>/dev/null || true
```

### How it runs

During `/torch-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `torch-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `torch-upgrade/migrations/` are
