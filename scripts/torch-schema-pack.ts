/**
 * torch-core@1.0.0 schema pack (T1 / Phase 0).
 *
 * Defines the 7 typed page kinds torch writes into a personal gbrain:
 *   torch/user-profile, torch/product, torch/goal, torch/developer-persona,
 *   torch/brand, torch/competitive-intel, torch/skill-run
 *
 * Plus the typed take kind torch writes for Phase 2 calibration:
 *   torch/take (kind=bet, holder=<user>, with expected_resolution_date)
 *
 ...
 * `mcp__gbrain__schema_graph` to render the torch subgraph correctly.
 */
...
export interface SchemaTypeDefinition {
  /** Page type slug, e.g. `torch/product`. */
  type: string;
...
const USER_PROFILE: SchemaTypeDefinition = {
  type: 'torch/user-profile',
  description:
    'Cross-project profile of the torch user: tone/conviction patterns, ' +
  ...
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/user-profile' },
    { name: 'slug', type: 'string', required: true, description: 'torch/user-profile/<user-slug>' },
  ...
  emits_links: [
    { verb: 'has_calibration', target_type: 'torch/take' },
  ],
};
...
const PRODUCT: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/product' },
    { name: 'slug', type: 'string', required: true, description: 'torch/product/<project-slug>' },
  ...
  emits_links: [
    { verb: 'targets', target_type: 'torch/goal' },
    { verb: 'observed_by', target_type: 'torch/developer-persona' },
    { verb: 'has_brand', target_type: 'torch/brand' },
    { verb: 'competes_with', target_type: 'torch/competitive-intel' },
    { verb: 'history', target_type: 'torch/skill-run' },
  ],
};
...
const GOAL: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/goal' },
    { name: 'slug', type: 'string', required: true, description: 'torch/goal/<project-slug>/<goal-id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'torch/product' },
  ],
};

const DEVELOPER_PERSONA: SchemaTypeDefinition = {
  type: 'torch/developer-persona',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/developer-persona' },
    { name: 'slug', type: 'string', required: true, description: 'torch/developer-persona/<project-slug>' },
...
const BRAND: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/brand' },
    { name: 'slug', type: 'string', required: true, description: 'torch/brand/<project-slug>' },
...
const COMPETITIVE_INTEL: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/competitive-intel' },
    { name: 'slug', type: 'string', required: true, description: 'torch/competitive-intel/<project-slug>' },
...
const SKILL_RUN: SchemaTypeDefinition = {
  type: 'torch/skill-run',
  description:
    'Every torch skill invocation that produces output writes one of these on completion. ' +
    'Time-series log of decisions, modes, mode-selected, outcomes. Powers /retro ' +
    'and (deferred) /torch-reflect. Auto-archives to summary-only after 90 days.',
  retention: 'archive-after-90d',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/skill-run' },
    { name: 'slug', type: 'string', required: true, description: 'torch/skill-run/<project>/<skill>/<timestamp>' },
  ...
  emits_links: [
    { verb: 'related_to', target_type: 'torch/product' },
    { verb: 'related_to', target_type: 'torch/goal' },
    { verb: 'writes_bet', target_type: 'torch/take' },
  ],
};
...
const TAKE: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'torch/take' },
    { name: 'slug', type: 'string', required: true, description: 'torch/take/<project>/<date>/<id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'torch/user-profile' },
    { verb: 'origin', target_type: 'torch/skill-run' },
  ],
};
...
export const torch_CORE_SCHEMA_PACK: SchemaPackJSON = {
  name: torch_SCHEMA_PACK_NAME,
  version: torch_SCHEMA_PACK_VERSION,
  page_types: [
    USER_PROFILE,
    PRODUCT,
    GOAL,
...
export function getSchemaPackMutationPayload(): {
  ...
  return {
    schema_pack: torch_CORE_SCHEMA_PACK,
...
export function getSchemaPackTypeNames(): ReadonlyArray<string> {
  return torch_CORE_SCHEMA_PACK.page_types.map((t) => t.type);
}
...
export function getRetentionPolicy(pageType: string): SchemaTypeDefinition['retention'] {
  const def = torch_CORE_SCHEMA_PACK.page_types.find((t) => t.type === pageType);
