#!/usr/bin/env bun
/**
 * analytics — CLI for viewing torch skill usage statistics.
 *
 * Reads ~/.torch/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.torch', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('torch skill usage analytics');
