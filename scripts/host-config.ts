...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/torch'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/torch'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $TORCH_ROOT env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/torch). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/torch'). */
    path: string;
  ...
  // --- Install Behavior ---
  install: {
    /** Whether torch-config skill_prefix applies (Claude only). */
    prefixable: boolean;
