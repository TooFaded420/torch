/**
 * torch-decision-semantic — OPTIONAL gbrain enhancement for decision resurfacing.
 *
 * This is the ONLY decision module that touches gbrain. The reliable core
 * (lib/torch-decision.ts) has zero gbrain imports and works with gbrain OFF; this
 * module is loaded lazily by `torch-decision-search` only on `--semantic`, and every
 ...
 *  - The curated-memory source is the one whose local_path is the torch brain
 *    worktree (`~/.torch-brain-worktree`), id `default` by convention — NOT a
 *    `torch-brain-<user>` id. Scoping search to it keeps code/doc corpora out.
 */
...
const BRAIN_WORKTREE_SUFFIX = ".torch-brain-worktree";
...
/**
 * Resolve the curated-memory source id (the torch brain worktree). Returns null
