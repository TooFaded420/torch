/**
 * torch-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/torch-memory-ingest.ts (Lane A)
 *   - bin/torch-gbrain-sync.ts   (Lane B)
 *   - bin/torch-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.torch/.gbrain-engine-cache.json (60s TTL)
 *
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[torch-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/torch-brain-sync (which only
...
function torchHome(): string {
  return process.env.torch_HOME || join(homedir(), ".torch");
}

function engineCachePath(): string {
  return join(torchHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(torchHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.torch/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "torch-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.torch/.gbrain-errors.jsonl. Never throws.
 ...
 * runtime read path used by torch-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(torchHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.torch/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
