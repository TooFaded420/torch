/**
 * torch-decision — event-sourced institutional decision memory.
 *
...
/** Resolve the per-project decision store paths. Bins pass slug + torch_HOME. */
export function decisionPaths(slug: string, torchHome?: string): DecisionPaths {
  const home = torchHome || process.env.torch_HOME || join(homedir(), ".torch");
