...
This document covers the command reference and internals of torch's headless browser.
...
torch's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.torch/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
The window has a subtle green shimmer line at the top edge and a floating "torch" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the torch icon in toolbar → Open Side Panel
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/torch/extension`
   - Dev/source: `<torch-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "torch browse"
6. **Set the port:** Click the torch icon → enter the port from `$B status` or `.torch/browse.json`
7. **Open Side Panel:** Click the torch icon → "Open Side Panel"
...
| **Connection pill** | Small "torch" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.torch/` inside the project root (detected via `git rev-parse --show-toplevel`).
...
| `BROWSE_STATE_FILE` | `.torch/browse.json` | Path to state file (CLI passes to server) |
...
The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. torch burns zero.
...
torch skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.torch/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/torch && git pull`
3. Rebuild: `cd ~/.claude/skills/torch && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/torch/browse/dist/browse`
