...
(function () {
  ...
  if (!Terminal) {
    console.error('[torch terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.torchPtySession so the
   ...
   * exposes the relevant fields on window.torchHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.torchHealth || {};
  }

  function getServerPort() {
    return window.torchServerPort || null;
  }

  function getAuthToken() {
    return window.torchAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your torch session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.torchPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`torch-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[torch terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.torchScanForPTYInject()
   ...
   * sync `const ok = torchInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.torchInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.torchScanForPTYInject = async function (text, origin) {
  ...
  async function getAuthTokenForScan() {
    if (window.__torchPtyScanToken) return window.__torchPtyScanToken;
    try {
      const resp = await fetch('http://127.0.0.1:34567/health');
      const body = await resp.json();
      const token = body.AUTH_TOKEN || body.authToken || '';
      if (token) window.__torchPtyScanToken = token;
      return token;
    } catch {
      return '';
    }
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[torch terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.torchPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`torch-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('torch:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.torchServerPort + window.torchAuthToken (which it does as soon
