...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'torch-status-pill';
  ...
  statusPill.innerHTML = `<span class="torch-pill-dot"></span> torch${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'torch-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'torch-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'torch-ref-panel';

  const header = document.createElement('div');
  header.className = 'torch-ref-panel-header';
  header.textContent = `torch refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'torch-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'torch-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'torch-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'torch-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'torch-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'torch-ref-panel-more';
...
// Do NOT dispatch torch-extension-ready here — the extension being loaded
