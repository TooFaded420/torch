...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"checkpoint","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/torch/bin/torch-timeline-log '{"skill":"checkpoint","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
eval "$(~/.claude/skills/torch/bin/torch-slug 2>/dev/null)" && mkdir -p ~/.torch/projects/$SLUG
CHECKPOINT_DIR="$HOME/.torch/projects/$SLUG/checkpoints"
