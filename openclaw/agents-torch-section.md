...
1. **Always spawn, never redirect.** When user asks to use ANY torch skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<torch-lite content>\n\n<task>")

**HEAVY:** needs a specific torch methodology
→ sessions_spawn(runtime: "acp", prompt: "Load torch. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /torch-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<torch-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<torch-plan content>\n\n<task>")
...
- "Upgrade torch", "update torch" → **HEAVY** with `Run /torch-upgrade`
