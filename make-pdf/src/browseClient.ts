 ...
 *   1. $torch_BROWSE_BIN env override (preferred, matches v1.24 torch_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: dirname(argv[0])/../browse/dist/browse[.exe]
 *   4. ~/.claude/skills/torch/browse/dist/browse[.exe]
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  // 1 + 2: env overrides (torch_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.torch_BROWSE_BIN ?? env.BROWSE_BIN;
  ...
  const globalPath = path.join(home, ".claude/skills/torch/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the torch Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $torch_BROWSE_BIN (${env.torch_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run torch setup from the torch repo:",
      "  cd ~/.claude/skills/torch && ./setup",
      "",
      "Or set torch_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx torch_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export torch_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
