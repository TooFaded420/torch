...
#       ~/.claude/skills/torch/bin/torch-config set local_code_index_offered true
#
# When silent: anything else (Path 1/2/3 users, anyone already on PGLite,
# anyone who opted out, anyone without remote-http MCP).
#
# Idempotency: writes a touchfile at ~/.torch/.migrations/v1.37.0.0.done
...
MIGRATIONS_DIR="$torch_HOME/.migrations"
DONE_TOUCH="$MIGRATIONS_DIR/v1.37.0.0.done"
CONFIG_BIN="$HOME/.claude/skills/torch/bin/torch-config"
...
if is_remote_http_mcp && is_local_engine_missing; then
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │  torch v1.37.0.0 — split-engine gbrain                          │
  │                                                                  │
  ...
  │  permanently:                                                    │
  │    torch-config set local_code_index_offered true               │
  └──────────────────────────────────────────────────────────────────┘
