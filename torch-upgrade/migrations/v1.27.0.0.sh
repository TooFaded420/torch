#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename torch-brain-* → torch-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename torch-brain-$USER →
#                              torch-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.torch-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.torch/config.yaml
...
#
# Interruption-safe via journal at ~/.torch/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.torch/.migrations/v1.27.0.0.done.
#
...
BIN_DIR="${SKILLS_DIR}/torch/bin"
CONFIG_BIN="${BIN_DIR}/torch-config"
URL_BIN="${BIN_DIR}/torch-artifacts-url"

MIGRATION_DIR="${torch_HOME}/.migrations"
...
OLD_REPO_NAME="torch-brain-${USER_NAME}"
NEW_REPO_NAME="torch-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.torch-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.torch-artifacts-remote.txt"
...
#   - presence of ~/.torch-brain-remote.txt (legacy file)
#   - presence of ~/.torch/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$torch_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy torch-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] torch-brain has been renamed to torch-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.torch-brain-remote.txt → ~/.torch-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.torch/config.yaml
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.torch-brain-remote.txt → ~/.torch-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.torch-brain-remote.txt → ~/.torch-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # torch-brain-$USER with torch-artifacts-$USER in the URL.
...
if ! journal_done "claude_md_block_rewritten"; then
  ...
  # Look in cwd's CLAUDE.md (where /setup-gbrain wrote it) and ~/.torch/CLAUDE.md
  ...
  for CMD in "$PWD/CLAUDE.md" "$torch_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$torch_HOME/.git" ]; then
    ...
    WORKTREE="${torch_BRAIN_WORKTREE:-$HOME/.torch-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.torch/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
