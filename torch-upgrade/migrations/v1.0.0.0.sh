...
# the older V0 prose can set `torch-config set explain_level terse`.
#
...
PROMPTED_FLAG="$torch_HOME/.writing-style-prompted"
PENDING_FLAG="$torch_HOME/.writing-style-prompt-pending"
...
EXPLAIN_LEVEL_SET="$("${HOME}/.claude/skills/torch/bin/torch-config" get explain_level 2>/dev/null || true)"
