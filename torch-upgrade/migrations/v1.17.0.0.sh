...
#
# Pre-1.17.0.0 /setup-gbrain wrote ~/.torch/consumers.json with a placeholder
...
#   - No ~/.torch/.git (brain-init never ran)
...
# CLI is a benign skip rather than blocking the rest of /torch-upgrade.
set -euo pipefail
...
WIREUP_BIN="${BIN_DIR}/torch-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: torch-config can emit trailing newlines,
