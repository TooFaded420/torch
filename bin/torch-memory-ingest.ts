#!/usr/bin/env bun
/**
 * torch-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.torch/ curated artifacts and writes
 ...
 *
 * Usage:
 *   torch-memory-ingest --probe                 # count what would ingest, no writes
 *   torch-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   torch-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   torch-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   torch-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.torch/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.torch/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.torch/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.torch/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.torch/analytics/eureka.jsonl                — typed: eureka
 *   ~/.torch/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.torch/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/torch-memory-helpers#secretScanFile (D19).
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (torch-brain-sync, git push)
...
const HOME = homedir();
const torch_HOME = process.env.torch_HOME || join(HOME, ".torch");
const STATE_PATH = join(torch_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: torch-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; torch-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.torch_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.torch_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "torch-memory-ingest",
      sessions: {},
    };
  }
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "torch-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in torch-gbrain-sync.ts:saveSyncState.
  try {
...
function* walktorchArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(torch_HOME, "projects");

  // Eureka log: ~/.torch/analytics/eureka.jsonl
  const eurekaLog = join(torch_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.torch/builder-profile.jsonl
  const builderProfile = join(torch_HOME, "builder-profile.jsonl");
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.torch/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.torch\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
 ...
 *
 *   The actual cross-machine exfiltration boundary is `torch-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/torch-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because torch-brain-sync already gates the cross-machine boundary
...
/**
 * Make a per-run staging directory at ~/.torch/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(torch_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.torch/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `gbrain import`, remote-http users get a stable path that survives.
 * torch-brain-sync's allowlist pushes ~/.torch/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.torch/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
 ...
 *
 * Reads ~/.claude.json directly (same fallback chain as torch-gbrain-detect
...
function cleanupStagingDir(dir: string): void {
  ...
  const verdict = checkOwnedStagingDir(dir, torch_HOME);
  if (!verdict.ok) {
    console.error(
  ...
  const forward = (signal: NodeJS.Signals) => () => {
    ...
    if (_activeStagingDir) {
      if (stagingDirIsCheckpointed(_activeStagingDir)) {
        ...
        // (in torch-gbrain-sync.ts) will see the checkpoint + dir and
 ...
 * configurable via torch_INGEST_TIMEOUT_MS (default 30 min, 1min–24h) so large
...
export function resolveImportTimeoutMs(
  raw: string | undefined = process.env.torch_INGEST_TIMEOUT_MS,
): number {
  if (raw === undefined || raw === "") return DEFAULT_IMPORT_TIMEOUT_MS;
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n) || Number.isNaN(n) || n < 60_000 || n > 86_400_000) {
    console.error(
      `[memory-ingest] torch_INGEST_TIMEOUT_MS="${raw}" invalid (need 60000–86400000ms); using ${DEFAULT_IMPORT_TIMEOUT_MS}ms`,
    );
...
function runGbrainImport(
  ...
  return new Promise((resolve) => {
    ...
    // parent torch-gbrain-sync seeding the bun grandchild's env).
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "torch-memory-ingest";
    saveState(state);
    return {
      written,
  ...
  // PERSISTENT dir under ~/.torch/transcripts/ and SKIP `gbrain import`
  // entirely. torch-brain-sync push will pick the dir up via its allowlist
  ...
  // torch_INGEST_RESUME_DIR (because gbrain's import-checkpoint.json points
  ...
  const resumeDir = process.env.torch_INGEST_RESUME_DIR;
  // #1802 second entry point: this binary is runnable directly, so it must not
  // trust torch_INGEST_RESUME_DIR just because it exists — a stale/poisoned env
  ...
  const resuming = !remoteHttpMode
    ...
    && checkOwnedStagingDir(resumeDir, torch_HOME).ok;
  if (!remoteHttpMode && resumeDir && resumeDir.length > 0 && !resuming) {
    console.error(
      `[memory-ingest] ignoring torch_INGEST_RESUME_DIR="${resumeDir}" — not a proven staging dir (#1802); staging fresh.`,
    );
  }
  ...
  try {
    ...
    // staged markdown lives under ~/.torch/transcripts/<run-id>/ and the
    // next torch-brain-sync push will move it to the artifacts repo. From
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "torch-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
      ...
      if (importResult.timedOut) {
        ...
        const msg = checkpointed
          ? `gbrain import timed out after ${mins}min; checkpoint preserved — re-run ` +
            `/sync-gbrain to resume (raise torch_INGEST_TIMEOUT_MS for big brains)`
          : `gbrain import timed out after ${mins}min before writing a checkpoint; ` +
            `re-run /sync-gbrain to restage (raise torch_INGEST_TIMEOUT_MS for big brains)`;
    ...
    // dir (makePersistentTranscriptDir, under ~/.torch/transcripts/) that
    // torch-brain-sync push must pick up — it is NOT a `.staging-ingest-*` dir
...
// The orchestrator runs it as `bun torch-memory-ingest.ts ...`, where
// import.meta.main is true, so the CLI path is unaffected.
if (import.meta.main) {
  main().catch((err) => {
    console.error(`torch-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  });
}
