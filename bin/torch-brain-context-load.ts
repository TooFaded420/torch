#!/usr/bin/env bun
/**
 * torch-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the torch preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   torch-brain-context-load --skill office-hours --repo garrytan-torch
 *   torch-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   torch-brain-context-load --window 14d --explain
 *   torch-brain-context-load --quiet
 */
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/torch-memory-helpers";
...
function printUsage(): void {
  console.error(`Usage: torch-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common torch skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "torch", args.skill, "SKILL.md"),
...
main().catch((err) => {
  console.error(`torch-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
