#!/usr/bin/env bun
/**
 * torch-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → torch-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → torch-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.torch/.sync-gbrain.lock (PID + start ts).
 ...
 * than building a torch-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/torch-memory-helpers";
...
const STATE_PATH = join(torch_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(torch_HOME, ".sync-gbrain.lock");
 ...
 * torch_HOME at runtime — same pattern as cacheFilePath() in
 * lib/gbrain-local-status.ts. Avoids the ESM static-import hoist trap where a
 * module-load-time const captures the real ~/.torch before a test can redirect.
 */
export function dreamMarkerPath(): string {
  return join(process.env.torch_HOME || join(homedir(), ".torch"), ".dream-in-progress");
}

// Default 35-minute timeout for code-walk + memory-ingest stages. Override via
// torch_SYNC_CODE_TIMEOUT_MS / torch_SYNC_MEMORY_TIMEOUT_MS. Bounds-checked
 ...
 */
export function decideResume(torchHome: string = torch_HOME): ResumeVerdict {
  const cp = readGbrainCheckpoint();
  if (!cp || !cp.dir) return { kind: "no-checkpoint" };
  const stagingDir = cp.dir;
  // #1802: only resume into a path we can PROVE is a torch-minted staging dir.
  ...
  const verdict = checkOwnedStagingDir(stagingDir, torchHome);
  if (!verdict.ok) {
...
function printUsage(): void {
  console.error(`Usage: torch-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the torch-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the torch-brain-sync git pipeline stage.
  ...
  --dream              Force the source-scoped dream cycle that builds this
                       ...
                       timeout 45min, override torch_SYNC_DREAM_TIMEOUT_MS.
 ...
 *
 * Pattern: `torch-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `torch_HOSTNAME` env override is honored for deterministic tests; in
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.torch_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("torch-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("torch-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("torch-code", `${base}-${hostPathHash}`);
}

/**
 * Pre-pathhash source id, kept for orphan detection only.
 *
 * Earlier /sync-gbrain versions registered `torch-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("torch-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("torch-code", base);
}
 ...
 * repo path: `torch-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("torch-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("torch-code", `${base}-${pathHash}`);
}
...
/**
 * Acquire the dream marker (`~/.torch/.dream-in-progress`). Returns false when
...
function skipStageForLocalStatus(
  ...
  const reasons: Record<Exclude<LocalEngineStatus, "ok">, string> = {
    ...
    "timeout":
      "engine probe timed out; raise torch_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow",
  };
...
function warnProbeTimeout(stage: "code" | "memory" | "dream"): void {
  process.stderr.write(
    `[torch-gbrain-sync] ${stage}: engine probe timed out — proceeding anyway; ` +
      `raise torch_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow\n`,
  );
}
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  // Earlier /sync-gbrain versions registered `torch-code-<slug>` (no path
  ...
  const codeTimeoutMs = resolveStageTimeoutMs(
    process.env.torch_SYNC_CODE_TIMEOUT_MS,
    "torch_SYNC_CODE_TIMEOUT_MS",
  );
  ...
  // torch's own .gitignore. Without the consumer-side entry, the pin gets
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: torch-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). torch-memory-ingest shells out
  ...
  if (resume.kind === "resume") {
    ...
    childEnv.torch_INGEST_RESUME_DIR = resume.stagingDir;
  ...
  const ingestPath = join(import.meta.dir, "torch-memory-ingest.ts");
  ...
  // .env.local footgun affects torch-memory-ingest.ts too, not just the
  ...
  const memoryTimeoutMs = resolveStageTimeoutMs(
    process.env.torch_SYNC_MEMORY_TIMEOUT_MS,
    "torch_SYNC_MEMORY_TIMEOUT_MS",
  );
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: torch-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "torch-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (torch-brain-sync not installed)" };
  }

  // #1731: torch-brain-sync is a bash shebang script; Windows can't spawn it
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `torch-brain-sync exited ${result.status}`,
  };
}
...
export async function runDream(args: CliArgs): Promise<StageResult> {
  ...
  try {
    const dreamTimeoutMs = resolveStageTimeoutMs(
      process.env.torch_SYNC_DREAM_TIMEOUT_MS,
      "torch_SYNC_DREAM_TIMEOUT_MS",
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "torch-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "torch-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "torch-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "torch-gbrain-sync"));
    }
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`torch-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
