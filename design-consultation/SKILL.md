...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the torch designer is available.
...
```bash
eval "$(~/.claude/skills/torch/bin/torch-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.torch/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
