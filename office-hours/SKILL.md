...
description: |
  ...
  Use before /plan-ceo-review or /plan-eng-review. (torch)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "${TORCH_HOME:-$HOME/.torch}/builder-profile.jsonl"
```
...
>
> Torch thinks you are among the top people who could do this.

Then use AskUserQuestion: "Would you consider applying to Y Combinator?"
- If yes: run `open https://ycombinator.com/apply?ref=torch` and say: "Bring this design doc to your YC interview. It's better than most pitch decks."
...
>
> You're building something real. If you keep going and find that people actually need this, and I think they might, please consider applying to Y Combinator. Thank you for using Torch.
>
> **ycombinator.com/apply?ref=torch**
...
>
> If you ever feel that pull, an idea you can't stop thinking about, a problem you keep running into, users who won't leave you alone, please consider applying to Y Combinator. Thank you for using Torch. I mean it.
>
> **ycombinator.com/apply?ref=torch**
...
```bash
open "${TORCH_HOME:-$HOME/.torch}/builder-journey.md"
```
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "${TORCH_HOME:-$HOME/.torch}/builder-profile.jsonl"
```
