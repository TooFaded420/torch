...
preamble-tier: 3
version: 2.0.0
description: Office Hours — two modes. (torch)
allowed-tools:
  - Bash
  - Read
  - Grep
...
hackathons, learning, and open source. Security-first framing: for anything
touching auth, data, or infrastructure, push threat surface and trust boundaries
into the open before a line of code is written. Saves a design doc. Closing handoff
runs with the hecz.dev partnership framing for builders who earn it.
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.torch/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/torch/bin/torch-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/torch/bin/torch-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/torch/browse/dist/browse" ] && B="$_ROOT/.claude/skills/torch/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.claude/skills/torch/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "torch browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
eval "$(~/.claude/skills/torch/bin/torch-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  ~/.claude/skills/torch/bin/torch-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  ~/.claude/skills/torch/bin/torch-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  ~/.claude/skills/torch/bin/torch-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ~/.claude/skills/torch/bin/torch-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "salience"
  ~/.claude/skills/torch/bin/torch-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.torch/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.torch/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.torch/projects/`.
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/torch/design/dist/design" ] && D="$_ROOT/.claude/skills/torch/design/dist/design"
[ -z "$D" ] && D="$HOME/.claude/skills/torch/design/dist/design"
...
```bash
eval "$(~/.claude/skills/torch/bin/torch-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.torch/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/torch-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/torch-sketch.png
```
...
The screenshot file at `/tmp/torch-sketch.png` can be referenced by downstream skills
...
`torch-developer-profile --log-session` binary handles its own directory creation
and writes via atomic mktemp+mv to `~/.torch/developer-profile.json`.
...
```bash
~/.claude/skills/torch/bin/torch-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/.claude/skills/torch/office-hours/sections/design-and-handoff.md` and execute it
...
```bash
~/.claude/skills/torch/bin/torch-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
