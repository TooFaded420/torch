...
describe('sidepanel.js: chat helpers ripped, terminal-injection helper survives', () => {
  ...
  test('Inspector "Send to Code" routes through the live PTY', () => {
    ...
    expect(sendBtn).toContain('window.torchInjectToTerminal');
  ...
  test('updateConnection no longer kicks off chat / tab polling', () => {
    ...
    expect(update).toContain('window.torchServerPort');
    expect(update).toContain('window.torchAuthToken');
  });
});

describe('sidepanel-terminal.js: eager auto-connect + injection API', () => {
  test('Exposes window.torchInjectToTerminal for cross-pane use', () => {
    expect(TERM_JS).toContain('window.torchInjectToTerminal');
    // Returns false when no live session, true when bytes go out.
    const inject = TERM_JS.slice(TERM_JS.indexOf('window.torchInjectToTerminal'));
  ...
  test('Repaint hook fires when Terminal pane becomes visible', () => {
    // The chat-tab rip removed torch:primary-tab-changed; we use a
