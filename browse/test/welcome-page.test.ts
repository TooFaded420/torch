...
describe('welcome page served via HTTP', () => {
  ...
  test('has torch Browser title and branding', () => {
    expect(html).toContain('<title>torch Browser</title>');
    expect(html).toContain('torch Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'torch-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('torch-status-pill');
