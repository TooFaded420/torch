...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['torch_PORT', 'torch_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses torch_PORT + torch_SKILL_TOKEN env when present', () => {
      process.env.torch_PORT = String(server.port);
      process.env.torch_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects torch_PORT env values with trailing characters', () => {
      process.env.torch_PORT = `${server.port}abc`;
      process.env.torch_SKILL_TOKEN = 'scoped-token';
