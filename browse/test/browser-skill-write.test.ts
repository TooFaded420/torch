 ...
 * never touch the user's real ~/.torch/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.torch/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.torch/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.torch', '.tmp');
});
