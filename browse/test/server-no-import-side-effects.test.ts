 ...
 * were registered. The subprocess uses HOME=tmp + torch_HOME=tmp so any
...
describe('server.ts module import has no auto-start side effects', () => {
  test('importing server.ts does not bind Bun.serve, register signal handlers, or write state', async () => {
    ...
    const tmptorch = path.join(tmpHome, '.torch');
...
// Check that the torch home directory wasn't populated as a side effect.
let torchPopulated = false;
try {
  const entries = fs.readdirSync(${JSON.stringify(tmptorch)});
  torchPopulated = entries.length > 0;
} catch {
...
`;
    ...
    const proc = Bun.spawn(['bun', '-e', childScript], {
      env: {
        ...
        // happens) won't crawl the host's real .torch/.
        BROWSE_STATE_FILE: path.join(tmptorch, 'browse.json'),
      },
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    // torch home should remain empty — initRegistry/initAuditLog/etc. side
    ...
    expect(snapshot.torchPopulated).toBe(false);
