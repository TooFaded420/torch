 ...
 *
 * Piggybacks on ~/.torch/analytics/skill-usage.jsonl pattern (existing
 * torch telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via torch-config.
 *
...
function analyticsDir(): string {
  return path.join(torchHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.torch_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // torch-config set telemetry off (preamble reads this; we trust the env hint).
