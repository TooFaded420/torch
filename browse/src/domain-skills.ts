 ...
 *
 * Scope:
 *   - per-project: ~/.torch/projects/<slug>/learnings.jsonl
 *   - global:      ~/.torch/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(torchHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(torchHome(), 'projects', slug, 'learnings.jsonl');
}
