/**
 * Terminal Agent — PTY-backed Claude Code terminal for the torch browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.torch', 'browse.json');
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.torch_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.torch_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.torch_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the torch browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the torch browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's torch
...
function buildServer() {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie torch_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `torch-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('torch-pty.') ? raw.slice('torch-pty.'.length) : raw;
        ...
        // Fallback: Cookie torch_pty (legacy / non-browser callers).
        if (!token) {
...
function main() {
  ...
  // sibling torch sessions. Callers (cli.ts spawn site, server.ts
