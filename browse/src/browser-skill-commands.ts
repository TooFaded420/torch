 ...
 * and passes it via torch_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    torch_PORT and torch_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get torch_PORT +
 * torch_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'torch_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip torch_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.torch_PORT = String(opts.port);
  out.torch_SKILL_TOKEN = opts.skillToken;
