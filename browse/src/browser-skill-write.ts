 ...
 *
 * /skillify stages a candidate skill into ~/.torch/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.torch/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.torch', '.tmp');
