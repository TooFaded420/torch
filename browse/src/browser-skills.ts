 ...
 *   project:  <project>/.torch/browser-skills/<name>/
 *   global:   ~/.torch/browser-skills/<name>/
 *   bundled:  <torch-install>/browser-skills/<name>/   (read-only, ships with torch)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.torch', 'browser-skills') : null,
    global: path.join(home, '.torch', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <torch-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <torch-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the torch root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with torch and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the torch source tree.
 */
