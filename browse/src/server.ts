...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.torch', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'torch', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'torch', 'browse', 'dist', 'browse'),
  ];
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.torch', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.torch', 'sidebar-agent-queue.jsonl');
  const torchDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(torchDir, { recursive: true, mode: 0o700 });
...
function killAgent(targetTabId?: number | null): void {
  ...
  const cancelDir = path.join(process.env.HOME || '/tmp', '.torch');
...
async function shutdown(exitCode: number = 0) {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.torch', 'chromium-profile');
...
async function start() {
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          ...
          const slug = process.env.TORCH_SLUG || 'unknown';
          ...
          // Fallback: built-in welcome page from torch install
          const skillRoot = process.env.TORCH_SKILL_ROOT || `${homeDir}/.claude/skills/torch`;
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        try {
          // Read ngrok authtoken: env var > ~/.torch/ngrok.env > ngrok native config
          let authtoken = process.env.NGROK_AUTHTOKEN;
          if (!authtoken) {
            const ngrokEnvPath = path.join(process.env.HOME || '', '.torch', 'ngrok.env');
  ...
  // Reads NGROK_AUTHTOKEN from env or ~/.torch/ngrok.env.
